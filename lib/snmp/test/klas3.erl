...
ftab(is_set_ok, [2], Cols) ->
%    io:format("calling ftab is_set_ok - fail~n"),
    {inconsistentValue, 2};
ftab(is_set_ok, _, Cols) ->
...
fname(get) ->
    Str2 = (catch begin
        case snmp:current_request_id() of
	    {value, Int} when integer(Int) -> ok;
	    {value, _} -> throw("bad_int");
	    _ -> throw("bad_req")
	end,
	case snmp:current_community() of
	    {value, Str} when list(Str) -> Str;
	    {value, _} -> throw("bad_str");
	    _ -> throw("bad_com")
	end,
	case snmp:current_address() of
	    {value, {[A,B,C,D], E}} when integer(E) -> ok;
	    {value, _} -> throw("bad_ip");
	    _ -> throw("bad_adr")
	end,
	case snmp:current_net_if_data() of
	    {value, []} -> ok;
