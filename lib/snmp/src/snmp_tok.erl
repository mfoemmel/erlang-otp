...
start_link(Reserved_words, Options) ->
	...
	{value, {file, Filename}} ->
		...
		{ok, Fid} ->
		    gen_server:start_link(snmp_tok,
	...
	false ->
	    MFA = case lists:keysearch(get_line_function, 1, Options) of
			  ...
			  {M,F,A};
		      false -> {snmp_tok, null_get_line, []}
		  end,
	    gen_server:start_link(snmp_tok,{Reserved_words,Options,MFA}, [])
    end.
...
%% Implementation
%%----------------------------------------------------------------------
insert_keywords_into_ets(DB, []) -> done;
...
%%    throw({error, {get(line), snmp_tok, "Unexpected eof~n"}}).
