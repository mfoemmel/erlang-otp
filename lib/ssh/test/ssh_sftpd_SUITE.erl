...
%%
sftp(doc) ->
    ["Start a daemon, and use shell sftp to connect to it."];
sftp(suite) ->
    [];
sftp(Config) when list(Config) ->
    ?line SysDir = ?config(data_dir, Config),
    ?line {ok, Pid} = ssh_sftpd:listen(9999, [{system_dir, SysDir}]),
    ?line Cmd = "sftp -oPort=9999 -oStrictHostKeyChecking=no localhost",
    ?line P = open_port({spawn, Cmd}, [stream]),
    ?line flush_debug_print_msgs(P),
    ?line _ = collect_data_upto(P, "sftp > "),
    ?line true = port_command(P, "cd /tmp\n"),
    ?line flush_debug_print_msgs(P),
    ?line _ = collect_data_upto(P, "sftp > "),
    ?line true = port_command(P, "ls\n"),
    ?line flush_debug_print_msgs(P),
    ?line _ = collect_data_upto(P, "sftp > "),
    ?line true = port_command(P, "mkdir sftpd_testing\n"),
    ?line flush_debug_print_msgs(P),
    ?line _ = collect_data_upto(P, "sftp > "),
    ?line true = port_command(P, "rmdir sftpd_testing\n"),
    ?line flush_debug_print_msgs(P),
    ?line _ = collect_data_upto(P, "sftp > "),
    ?line true = port_command(P, "exit\n"),
    ?line flush_all_data(P),
    ?line ok = ssh_sftpd:stop(Pid).
