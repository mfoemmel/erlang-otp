...
%% External exports
-export([start/0, start_link/1, stop/1, get_collector_pid/1]).
...
%%   {width, integer()}
%%   {height, integer()}
%%   {collector_pid, pid()} | {collector_pid, undefined}
%%   {active_filter, atom()}
%%   {max_events, integer()} | {max_events, infinity} 
%%   {max_actors, integer()} | {max_actors, infinity} 
%%   {actors, Actors}
%%   {first_event, event_key()}
%%   {hide_unknown, boolean()}
%%   {hide_actions, boolean()}
%%   {mode, all} | {mode, {search_actors, Dir, FirstKey, Actors}}
%%   Dir = forward | reverse
%%   FirstKey = event_key()
%%   Actors = [term()]
%%   Level = integer() | min | max
%%----------------------------------------------------------------------

start() ->
    {ok, ViewerPid} = et_viewer:start_link([{trace_global, true}]),
...
default_state() ->
    #state{parent_pid     = self(),
           ...
           filters        = [#filter{name = collector,
                                     function = fun(E) -> et:make_event(E) end}],
...
parse_opt([H | T], S, CollectorOpt) ->
	...
	{trace_client, Client} -> 
        ...
        {mode, Mode = {search_actors, Dir, Key, Actors}} when list(Actors), Dir == forward ->
            parse_opt(T,  S#state{display_mode = Mode}, CollectorOpt);
        {mode, Mode = {search_actors, Dir, Key, Actors}} when list(Actors), Dir == reverse ->
            parse_opt(T,  S#state{display_mode = Mode}, CollectorOpt);

        Bad ->
            {error, {bad_option, Bad}}
    end;
parse_opt(BadList, S, _) ->
...
do_dict_delete(Key, S) ->
    %% ok = error_logger:format("~p(~p): handle_info({et, {dict_delete, ~p}})~n",
    %%                          [?MODULE, self(), Key]),
    S.

%%----------------------------------------------------------------------
%% Returns the process identifier of the collector
%%----------------------------------------------------------------------
...
handle_call(get_collector_pid, From, S) ->
                ...
                case S#state.actors of
                    ...
                    Actors ->
                        ...
                        case New#actor.name == Old#actor.name of
                            ...
                            false ->
                                move_actor(Old, New, Actors, CollectorPid, S)
                        end
                end;
            {event, N} ->
                %% Event click ignored
...
change_display_mode(Mode, S) ->
    case Mode of
        ...
        {search_actors, _Dir, _Key, []}  ->
            S2 = S#state{display_mode = all},
            refresh_main_window(S2);
        {search_actors, Dir, Key, Actors} when list(Actors) ->
            Pid = S#state.collector_pid,
            Prev = et_collector:iterate(Pid, Key, -1),
...
display_more_events(Try, S) ->
    ...
    case queue_length(S2#state.events) - queue_length(S#state.events) of
        Diff when Diff == Try ->
            ...
            {noreply, S2, 0};
        Diff when S2#state.first_event == S#state.first_event,
...
opt_display_event(E, S) ->
    case S#state.display_mode of
        all ->
            display_event(E, S);
        {search_actors, _Dir, _Key, Actors} ->
...
do_ensure_actor(Name, S, [H | T], N) when H#actor.name == Name ->
    Pos = (?initial_x + (N * ?incr_x)) * S#state.scale,
    {false, {Name, Pos, S}};
do_ensure_actor(Name, S, [H | T], N) ->
...
move_actor(From, To, Actors, CollectorPid, S) ->
    Pos      = #actor.name,
