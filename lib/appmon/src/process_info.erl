...
-export([get_nodes/0,is_node/1,get_process_data/2,
	 get_application_keys/2]).
...
handle_call({host_name},_From,State)->
    {reply,get_host_name(),State};

handle_call({applications,Node},_From,State)->
    {reply,get_name_of_applications(Node),State};

handle_call({application_keys,App,Node},_From,State)->
    {reply,retrieve_application_keys(App,Node),State};

handle_call({procs,App,Mode,Node},_From,State) ->
    {reply,{get_application_info(App,Mode,Node),State},State};

handle_call({process_data,Pid,Node},_From,State) ->
    {reply,get_process_info(Pid,Node),State};
  
handle_call({send_trace,Proc},_From,State) ->
...
handle_call({is_node,Node},_From,State) ->
...
get_name_of_applications(Node)->
    ...
    lists:filter(Del_not_run_fun,
		 (lists:map(Get_elem_fun,
			    (rpc:call(Node,
				      application,
				      which_applications,[]))))).
...
%%                                                                    %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Retrieve the processes in a application
...
%%The tought is
%% 1. Get the processes that Pid links to Pid.
%%    Pid is the application_master the first time
%% 2. Add the Process to the database and clear the list of children
%%    from processes Which for some resason not should be there.
%% 3. Queue the children, so we later can se if they has any links 
%% 4. Add links to the childrens
%% 5. When the whole tree is retreived remove the unnecessary processes
%%    depending on the mode
%% 6. Take all links that points to the same Pid and sort out
...
%%The Pid is now the application_master and the application masters
%% child is the Application supervisor but in reality there are 2
...
get_childs(Port,_Mode,_Node) when port(Port)-> 
    [].
...
%% The first case when its the master process there are only one real
%% child even thogh thre arer more links
add_childs({master,Pid},Db,[Child|_Rest],N)->
    add_child(Pid,Db,Child,N);

add_childs(_Pid,_Db,[],_N)->
    ok;
add_childs(Pid,Db,[Child|Rest],N)->
    add_child(Pid,Db,Child,N),
    add_childs(Pid,Db,Rest,N+1).
...
%%second time. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
remove_used_childs([],_Db,New_list)->
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%take the list of links and separate it to a list with ports and a
...
%% the list of childs from processes that not shuold be there
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%This is the first case no childs are used so thats not necessary
add_and_remove(Childs,{master,Pid},Db,_Grp,Node) when
  pid(Pid), Node==node(Pid)->
    ets:insert(Db#data.procs,{Pid,{master,master},controller}),
    {_Pids,Ports}=separate_ports(Childs,[],[]),
    Ports++Childs;

%%This clause is removable when using only link as retrieving mode  
add_and_remove(Childs,{Pid,_Name},Db,Grp,Node) when
  pid(Pid),Node==node(Pid)->
    ets:insert(Db#data.procs,
	       {Pid,rpc:call(Node,erlang,process_info,
			     [Pid,registered_name])}), 
...
add_and_remove(_Childs,Pid,_Db,_Grp,Node)when pid(Pid),Node/=node(Pid)->
    [];

%%take care of the ports donmt add them 
%%to the table with processes 
add_and_remove(_Childs,Pid,_Db,_Grp,_Node)when port(Pid)->
    [].

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Control that the applications groupleader is the process Pids
%%groupleader
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Add the links that points to the same child and not child to a
%% supervisor to the shared links table 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%N_links contains the a list of lists where each list is a list of
%% the links that points to the same child
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%This method should be used as a fun to the filterfunction in
%% take_sup_links.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Mark the processes that is in the supervisor tree as either worker or
%%supervisor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%the supervisr tree, get the processes in the supervisortree.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Delete the processes that is in the supervisortree but workers and
%% their links
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
delete_workers(Db) ->
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Build a node and then its runs itself on every child to the current
%%% pid
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
build_node(Pid,Db,Node)when pid(Pid),Node==node(Pid)->
    ...
    {get_name(Pid,Db),lists:map(Build_tree_fun,Childs1),
     lists:map(Fix_sec_name_fun,Sec_childs) };

build_node(Pid,_Db,Node)when pid(Pid),Node/=node(Pid)->
    {"Runs on another node:" ++ erlang:pid_to_list(Pid) ,[],[]};
build_node(Pid,_Db,_Node)when port(Pid)->
    {"Port :" ++ erlang:port_to_list(Pid) ,[],[]}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Select the name of the Pid frpm the database where we previosly     %%
%%added it                                                            %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
get_name(Pid,Db)->    
...
%%When port..
get_link_name({_,Pid,_},_Db)when port(Pid)->    
    "Port :" ++ " : ";
get_link_name(_,_)->
    "".
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%The main function to handle the trace signal, Controls if           %%
%% the Pricess is in teh table  whit traced processes if so it stop   %%
%% the trace otherwise it starts trace the process                    %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Control that the processess in the ets:table with traced processes  %%
%%is alive if not remove them                                         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
get_key(Pid)->
    case rpc:call(node(Pid),
		  erlang,process_info,[Pid,registered_name]) of
	[]->
	    pid_to_list(Pid);
