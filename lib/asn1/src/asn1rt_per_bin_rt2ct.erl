...
dec_fixup([novalue|T],[Hc|Tc],RemBytes,Acc) ->
    dec_fixup(T,Tc,RemBytes,Acc);
dec_fixup([{_Name,novalue}|T],[Hc|Tc],RemBytes,Acc) ->
    dec_fixup(T,Tc,RemBytes,Acc);
...
getbits_as_binary(0,Buffer) ->
    {{0,<<>>},Buffer};
getbits_as_binary(Num,Buffer={0,Bin}) when Num > 16 ->
    Used = Num rem 8,
    Pad = (8 - Used) rem 8,
    Nbytes = Num div 8,
    <<Bits:Num,_:Pad,RestBin/binary>> = Bin,
    {{Pad,<<Bits:Num,0:Pad>>},RestBin};
getbits_as_binary(Num,Buffer={Used,Bin}) -> % Unaligned buffer
    %% Num =< 16,
...
getoctets({0,Buffer},Num) ->
    ...
    {Val,{0,RestBin}};
getoctets({U,<<Padding,Rest/binary>>},Num) when U /= 0 ->
...
%%
set_choice(Alt,{L1,L2},{Len1,Len2}) ->
...
set_choice_tag(Alt,[Alt|Rest],Tag) ->
    Tag;
set_choice_tag(Alt,[H|Rest],Tag) ->
    set_choice_tag(Alt,Rest,Tag+1);
set_choice_tag(Alt,[],Tag) ->
    false.
...
decode_fragmented_bits({N,<<B,Bs/binary>>},C) ->
...
decode_fragmented_octets({N,<<B,Bs/binary>>},C) ->
...
encode_integer([{'ValueRange',{'MIN',_}}],Val) ->
    encode_unconstrained_number(Val);
encode_integer([{'ValueRange',VR={Lb,Ub}}],Val) ->
...
encode_constrained_number({Lb,Ub},Range,{bits,N},Val) ->
    Val2 = Val-Lb,
%    {bits,N,Val2};
    [10,N,Val2];
encode_constrained_number({Lb,Ub},Range,{octets,N},Val) when N < 256->
    %% N is 8 or 16 (1 or 2 octets)
    Val2 = Val-Lb,
%    {octets,<<Val2:N/unit:8>>};
    [20,N,Val2];
encode_constrained_number({Lb,Ub},Range,{octets,N},Val) -> % N>255
    %% N is 8 or 16 (1 or 2 octets)
    Val2 = Val-Lb,
%    {octets,<<Val2:N/unit:8>>};
    [21,<<N:16>>,Val2];
encode_constrained_number({Lb,Ub},Range,_,Val) ->
    Val2 = Val-Lb,
    if
...
decode_constrained_number(Buffer,{Lb,Ub},Range,{bits,N}) ->
    {Val,Remain} = getbits(Buffer,N),
    {Val+Lb,Remain};
decode_constrained_number(Buffer,{Lb,Ub},Range,{octets,N}) ->
    {Val,Remain} = getoctets(Buffer,N),
    {Val+Lb,Remain}.

decode_constrained_number(Buffer,{Lb,Ub},Range) ->
						%    Val2 = Val - Lb,
    {Val,Remain} = 
	if 
	    Range  == 2 ->
...
decode_length(Buffer,{Lb,Ub}) when integer(Lb), Lb >= 0 -> % Ub > 65535
...
% X.691:10.9.3.5 
decode_length({Used,Bin},{_,_Lb,Ub}) -> %when Len =< 127 -> % Unconstrained or large Ub NOTE! this case does not cover case when Ub > 65535
    Unused = (8-Used) rem 8,
    case Bin of
	...
	<<_:Used,_:Unused,2:2,Val:14,Rest/binary>> -> 
	    {Val, {0,Rest}};
	<<_:Used,_:Unused,3:2,_:14,Rest/binary>> -> 
...
decode_enumerated(Buffer,C,{Ntup1,Ntup2}) when tuple(Ntup1), tuple(Ntup2) ->
	...
	0 -> % not an extension value
		...
		Error -> exit({error,{asn1,{decode_enumerated,{Val,[Ntup1,Ntup2]}}}})
	    end;
...
decode_enumerated(Buffer,C,NamedNumberTup) when tuple(NamedNumberTup) ->
	...
	Error -> exit({error,{asn1,{decode_enumerated,{Val,NamedNumberTup}}}})
    end.
...
encode_bit_string(C, LoNB=[FirstVal | RestVal], NamedBitList) when atom(FirstVal) ->
    ToSetPos = get_all_bitposes(LoNB, NamedBitList, []),
    BitList = make_and_set_list(ToSetPos,0),
    encode_bit_string(C,BitList,NamedBitList);% consider the constraint

encode_bit_string(C, BL=[{bit,No} | RestVal], NamedBitList) ->
...
encode_bit_string(C, BitListValue,[]) 
  when list(BitListValue) ->
    [encode_length(C,length(BitListValue)),
     2,BitListValue];
encode_bit_string(no, BitListValue,NamedBitList) 
  when list(BitListValue) ->
...
bit_string_trailing_zeros1(BitList,Lb,Ub) ->
	...
	D -> F = fun(L,LB,LB,_,_)->lists:reverse(L);
		    ...
		    (_,L1,_,_,_) ->exit({error,{list_length_BIT_STRING,
						 BitList}}) end,
...
decode_bit_string(Buffer, C, NamedNumberList) ->
	...
	V when integer(V) ->
	    Bytes2 = align(Buffer),
	    {BinBits,Bytes3} = decode_fragmented_bits(Bytes2,V),
...
bit_list_or_named1(Pos,[],Names,Acc) ->
    lists:reverse(Acc).
...
chars_decode(Bytes,_,'BMPString',_,Len) ->
    getBMPChars(Bytes,Len);
chars_decode(Bytes,NumBits,StringType,CharInTab,Len) ->	
...
chars_decode2(Bytes,{Min,Max,CharInTab},NumBits,Len,Acc) ->
    ...
    chars_decode2(Bytes2,{Min,Max,CharInTab},NumBits,Len -1,[element(Char+1,CharInTab)|Acc]).
						...
						% X.691:17 
encode_null(Val) -> []; % encodes to nothing
...
% complete(L) ->
% %    List = [[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[],10,1,0],10,1,0],10,7,1],10,1,0],10,3,0],10,1,1],2],20,4,[124,124,124,222]],20,2,<<217,3>>],10,1,1],20,1,[1]],10,1,0],10,2,2],10,1,0],10,1,0],10,2,1],20,2,[39,15]],10,1,1],20,1,[1]],10,2,0],10,2,0],20,1,[0]],20,1,[1]],10,1,0],10,3,2],10,1,0],10,1,0],20,1,[1]],10,1,0],20,1,[0]],10,3,2],2],20,3,[255,0,0]],
% %    List = [[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[],10,1,0],10,1,0],10,7,1],10,1,0],10,3,0],10,1,1],2],20,4,[123,123,123,4]],20,2,<<217,3>>],10,1,1],20,1,[1]],10,1,0],10,2,0],10,1,0],10,2,1],20,2,[39,17]],20,1,[1]],10,2,0],10,2,0],20,1,[0]],20,1,[1]],10,1,0],10,2,0],10,1,0],10,3,2],10,1,0],20,1,[1]],10,1,0],20,1,[0]],10,3,2],2],20,3,[255,0,0]],20,1,[3]],10,1,0],10,3,3],10,1,0],10,2,1],20,2,[8,175]],20,1,[2]],10,1,0],10,2,0],2],20,4,[0,9,0,4]],20,1,[0]],10,1,0],10,2,1],2],20,4,[0,4,0,4]],10,1,0],10,4,4],10,1,0],10,8,0],10,8,0],20,1,[0]],10,1,0],10,3,5],20,1,[1]],10,1,0],10,1,0],10,1,0],10,5,0],2],20,4,[0,5,0,49]],20,1,[0]],10,1,0],10,3,6],10,2,3],10,8,0],10,8,0],10,1,0],10,3,7],10,7,1],10,7,23],10,7,99],20,1,[65]],2],10,8,40],10,8,48],10,8,115],10,8,124],10,8,32],10,8,48],10,8,48],10,8,115],10,8,124],10,8,91],10,8,49],10,8,45],10,8,55],10,8,93],10,8,120],10,8,108],10,8,120],10,8,120],10,8,124],10,8,56],10,8,108],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,124],10,8,35],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,124],10,8,42],10,8,120],10,8,120],10,8,124],10,8,57],10,8,108],10,8,49],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,120],10,8,124],10,8,57],10,8,108],10,8,48],10,8,49],10,8,49],10,8,120],10,8,46],10,8,115],10,8,41],
%     List = [[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[],10,1,0],10,1,0],10,7,1],10,1,0],10,3,0],10,1,1],2],20,4,[125,125,125,111]],20,2,<<217,3>>],10,1,1],20,1,[1]],10,1,0],10,2,2],10,1,0],10,1,0],10,2,1],20,2,[195,87]],10,1,1],20,1,[1]],10,2,0],10,2,0],20,1,[0]],20,1,[1]],10,1,0],10,3,5],10,1,0],10,2,2],10,1,0],20,1,[0]],10,3,2],2],20,3,[255,255,255]],20,1,[3]],10,1,0],10,4,1],10,1,0],10,2,1],10,1,1],20,1,[1]],20,2,<<0,1>>],10,1,0],10,3,7],10,1,0],10,3,4],0],10,3,2],20,1,[1]],10,1,0],10,1,0],2],20,4,[0,11,0,7]],20,1,[1]],20,1,[2]],2],20,2,[52,48]],10,1,0],20,1,[1]],20,1,[4]],10,1,0],10,1,0],2],20,4,[0,0,176,1]],20,1,[1]],20,1,[1]],2],20,1,[48]],10,1,0],10,1,0],2],20,4,[0,0,176,8]],20,1,[1]],20,1,[22]],2],20,22,[73,78,32,73,80,52,32,49,50,53,46,49,50,53,46,49,50,53,46,49,49,49]],10,1,0],10,1,0],2],20,4,[0,0,176,15]],20,1,[1]],20,1,[21]],2],20,21,[97,117,100,105,111,32,49,49,49,49,32,82,84,80,47,65,86,80,32,32,52]],10,1,0],10,1,0],2],20,4,[0,0,176,12]],20,1,[1]],20,1,[8]],2],20,8,[112,116,105,109,101,58,51,48]],10,1,0],20,1,[1]],20,1,[4]],10,1,0],10,1,0],2],20,4,[0,0,176,1]],20,1,[1]],20,1,[1]],2],20,1,[48]],10,1,0],10,1,0],2],20,4,[0,0,176,8]],20,1,[1]],20,1,[22]],2],20,22,[73,78,32,73,80,52,32,49,50,52,46,49,50,52,46,49,50,52,46,50,50,50]],10,1,0],10,1,0],2],20,4,[0,0,176,15]],20,1,[1]],20,1,[21]],2],20,21,[97,117,100,105,111,32,50,50,50,50,32,82,84,80,47,65,86,80,32,32,52]],10,1,0],10,1,0],2],20,4,[0,0,176,12]],20,1,[1]],20,1,[8]],2],20,8,[112,116,105,109,101,58,51,48]],10,1,0],10,4,10],20,1,[2]],10,1,0],10,8,0],10,8,11],10,7,1],10,1,0],10,8,0],10,8,11],10,7,1],10,1,0],10,4,9],20,1,[7]],10,1,1],2],20,4,[0,12,0,4]],20,1,[1]],20,1,[4]],2],20,4,[49,50,48,48]],10,1,1],2],20,4,[0,11,0,2]],20,1,[1]],20,1,[5]],2],20,5,[54,50,51,48,48]],10,1,1],2],20,4,[0,12,0,5]],20,1,[1]],20,1,[3]],2],20,3,[55,48,48]],10,1,1],2],20,4,[0,11,0,3]],20,1,[1]],20,1,[5]],2],20,5,[52,53,49,48,48]],10,1,1],2],20,4,[0,12,0,6]],20,1,[1]],20,1,[3]],2],20,3,[48,46,50]],10,1,1],2],20,4,[0,12,0,7]],20,1,[1]],20,1,[2]],2],20,2,[50,48]],10,1,1],2],20,4,[0,12,0,8]],20,1,[1]],20,1,[2]],2],20,2,[52,48]],
%     case catch port_control(drv_complete,1,List) of
% 	Bin when binary(Bin) ->
% 	    Bin;
% 	{'EXIT',{badarg,Reason}} ->
% 	    case start_drv(self()) of
% 		ok ->
% 		    port_control(drv_complete,1,List);
% 		Error -> % driver_error
...
% complete1([{octets,Bin}|T],Acc) when binary(Bin),size(Bin)<256 ->
%     complete1(T,[Acc|[20,size(Bin),Bin]]);

% complete1([{octets,Bin}|T],Acc) when binary(Bin) ->
%     B = size(Bin),
%     complete1(T,[Acc|[21,<<B:16>>,Bin]]);

% complete1([{octets,Bin}|T],Acc) when list(Bin),length(Bin)<256 ->
%     complete1(T,[Acc|[20,length(Bin),Bin]]);

% complete1([{octets,Bin}|T],Acc) when list(Bin) ->
%     L = length(Bin),
%     complete1(T,[Acc|[20,<<L:16>>,Bin]]);

% complete1([{debug,_}|T], Acc) ->
%     complete1(T,Acc);

% complete1([{bits,N,Val}|T],Acc) ->
%     complete1(T,[Acc|[10,N,Val]]);

% complete1([{bit,Val}|T],Acc) ->
%     complete1(T,[Acc|[Val]]);
...
% complete1([align|T],Acc) ->
%     complete1(T,[Acc|[2]]);
% complete1([{0,Bin}|T],Acc) when binary(Bin),size(Bin)<256 ->
%     complete1(T,[Acc|[30,0,size(Bin),Bin]]);
% complete1([{Unused,Bin}|T],Acc) when integer(Unused),binary(Bin),size(Bin)<256 ->
%     complete1(T,[Acc|[30,Unused,size(Bin),Bin]]).


-endif.

%% start_drv/1
%% This function loads the driver, opens and registers a port 
%% used to invoke the driver
start_drv(Caller) ->
%%    io:format("loading driver ...~n"),
    PrivDir=code:priv_dir(asn1),
    case erl_ddll:load_driver(filename:join([PrivDir,"bin"]),
					     "asn1_erl_drv") of
	ok -> 
	    start_drv2(Caller);
...
start_drv2(Caller) ->
    spawn(?MODULE,init_drv,[Caller]),
    receive 
	driver_ready ->
	    ok;
	Error={port_error,Reason} ->
	    Error %% if port could not be opened
    end.

init_drv(Caller) ->
    case catch open_port({spawn,"asn1_erl_drv"},[]) of
	{'EXIT',Reason} ->
	    Caller ! {port_error,Reason};
	Port ->
	    register(drv_complete,Port),
	    Caller ! driver_ready
    end,
    receive
	stop-> % when driver is unloaded
	    ok
    end.
