			       ...
			       const char* buf, int* index)
{
	...
	if (ei_get_type(buf, index, &ty, &n) < 0) goto err;
	if (ty != ERL_NIL_EXT) {
    ...
    return ch_written;
 err:
    erl_err_msg("<ERROR> ei_print_term: Bad type of term !");
    return -1;
}
...
int ei_skip_term(const char* buf, int* index)
{
	...
	if (ei_decode_atom(buf, index, NULL) < 0)
	   goto err;
	break;
    case ERL_PID_EXT:
	if (ei_decode_pid(buf, index, NULL) < 0) goto err;
	break;
    case ERL_PORT_EXT:
	if (ei_decode_port(buf, index, NULL) < 0) goto err;
	break;
    case ERL_NEW_REFERENCE_EXT:
    case ERL_REFERENCE_EXT:
	if (ei_decode_ref(buf, index, NULL) < 0) goto err;
	break;
    case ERL_NIL_EXT:
	if (ei_decode_list_header(buf, index, &n) < 0) goto err;
	break;
	...
	break;
    case ERL_STRING_EXT:
	if (ei_decode_string(buf, index, NULL) < 0) goto err;
	break;
    case ERL_SMALL_TUPLE_EXT:
    case ERL_LARGE_TUPLE_EXT:
	if (ei_decode_tuple_header(buf, index, &n) < 0) goto err;	
	...
	break;
    case ERL_FUN_EXT:
	if (ei_decode_fun(buf, index, NULL) < 0) goto err;
	break;
    default:
	goto err;
    }
    return 0;
 err:
    erl_err_msg("<ERROR> ei_skip_term: unknown type of term !");
    return -1;
}
