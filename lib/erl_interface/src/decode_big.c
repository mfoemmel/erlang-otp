...
extern int ei_decode_big(const char *buf, int *index, erlang_big *b)
{
  long digit_bytes;
...
erlang_big *ei_alloc_big(int arity)
{
    ...
    if ( (b = erl_malloc(sizeof(erlang_big))) == NULL) return NULL;
    memset(b,(char)0,sizeof(erlang_big));
    if ( (b->digits = erl_malloc(arity*2)) == NULL) {
        erl_free(b);
        return 0;
    }
...
static INLINE int erts_check_x87(double f)
{
