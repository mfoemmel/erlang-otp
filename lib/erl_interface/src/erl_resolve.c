 ...
 */
erl_mutex_t *erl_gethost_sem = NULL;
#ifndef __WIN32__
int h_errno;
#endif
...
void erl_init_resolve(void)
{
  ...
  erl_gethost_sem = erl_mutex_create();
}
				...
				int *h_errnop)
{
  ...
  /* this should have been done in erl_init() */
  if (!erl_gethost_sem) erl_init_resolve(); 

  /* === BEGIN critical section === */
  if (erl_mutex_lock(erl_gethost_sem,0) != 0) {
    *h_errnop = NO_RECOVERY;
    return NULL;
  }

  /* lookup the data */
  if ((src = erl_gethostbyname(name))) {
  ...
  erl_mutex_unlock(erl_gethost_sem);
  return rval;
}
				...
				int *h_errnop)
{
  ...
  /* lookup the data */
  if ((src = erl_gethostbyaddr(addr,length,type))) {
    /* copy to caller's buffer */
    if (!copy_hostent(&dest,src,buffer,buflen)) {
      /* success */
      *hostp = dest;                
...
#else /* unix of some kind */
/*
 * If we don't have libresolv, we only have one version to use and can
 * short circuit the two tests.
 */

#ifdef HAVE_RES_GETHOSTBYNAME
static int erl_use_resolver;
static int initialized, use_resolver;

static void initialize()
{
  extern void erl_exit();	/* in global.h */

  char *s = getenv("ERL_RESOLV");
  if (s) {
    if (!strcmp(s, "nodns")) use_resolver = 0;
    else use_resolver = 1;
  }
  else {
    use_resolver = erl_use_resolver;
  }
  initialized = 1;
}
...
{
#ifdef HAVE_RES_GETHOSTBYNAME
  extern struct hostent *res_gethostbyname();

  if (!initialized) initialize();

  if (use_resolver) return res_gethostbyname(name);
  else 
#endif /* HAVE_RES_GETHOSTBYNAME */
    return gethostbyname(name);
}
...
struct hostent *erl_gethostbyaddr(const char *addr, int len, int type)
{
  ...
  if (use_resolver) return res_gethostbyaddr(addr, len, type);
  else 
#endif /* HAVE_RES_GETHOSTBYNAME */
    return gethostbyaddr(addr, len, type);
}
