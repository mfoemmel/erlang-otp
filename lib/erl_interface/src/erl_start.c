...
static unsigned long spawn_erlang_epmd(char *alive,
				       Erl_IpAddr adr,
				       int flags, 
				       char *erl_or_epmd, 
				       char *args[], 
				       ...
				       int is_erlang);
#else
static int exec_erlang(char *alive, Erl_IpAddr adr, int flags, char *erl, char *args[],int port);
#endif
...
{
#if 0 /* for now - to get it to compile */
  return ERL_NO_DAEMON;
#else
  ...
  if (bind(sockd,(struct sockaddr *)&addr,sizeof(addr))<0) return ERL_SYS_ERROR;
  namelen = sizeof(addr);
  if (getsockname(sockd,(struct sockaddr *)&addr,&namelen)<0) return ERL_SYS_ERROR;
  port = ntohs(addr.sin_port);

  listen(sockd,5);

#if defined(VXWORKS) || defined(__WIN32__)
  if((pid = spawn_erlang_epmd(alive,adr,flags,erl,args,port,1)) 
      == 0)
  ...
  case 0:
    ...
    exec_erlang(alive, adr, flags, erl, args, port);
...
static int enquote_args(char **oargs, char ***qargs){
	...
	**qargs = NULL;
	...
	;
    args = erl_malloc(sizeof(char *) * (len + 1));
	...
	for(ptr = oargs[i]; *ptr != '\0'; ++ptr)
	    extra += (*ptr == '"');
	args[i] = erl_malloc(strlen(oargs[i]) +
			     extra +
			     1);
	ptr2 = args[i];
	if(qwhole)
				       ...
				       int is_erlang)
{
    ...
    if(is_erlang){
	get_addr(erl_thishostname(), &myaddr);
#if defined(VXWORKS)
 ...
 */
static int exec_erlang(char *alive, 
		       Erl_IpAddr adr,
		       int flags, 
		       char *erl, 
		       char *args[], 
		       int port)
{
  ...
  *argbuf=(char)0;
  ...
  if (flags & ERL_START_VERBOSE) {
    fprintf(stderr,"erl_start: %s %s %s\n",argv[0],argv[1],argv[2]);
  }
  ...
  if (flags & ERL_START_DEBUG) {
    fprintf(stderr,"erl_start: exec failed: (%d) %s %s %s\n",
...
#ifdef DEBUG
  fprintf(stderr,"time is %ld.%06ld, will timeout at %ld.%06ld\n",
...
#ifdef DEBUG
    fprintf(stderr,"remaining to timeout: %ld.%06ld\n",to.tv_sec,to.tv_usec);
#endif
...
#ifdef DEBUG
      gettimeofday(&now,NULL);
      fprintf(stderr,"timed out at %ld.%06ld\n",now.tv_sec,now.tv_usec);
#endif
      return ERL_TIMEOUT;
      break;
...
#ifdef DEBUG
      gettimeofday(&now,NULL);
      fprintf(stderr,"got select at %ld.%06ld\n",now.tv_sec,now.tv_usec);
#endif
...
int erl_start_epmd(void)
{
    ...
    unsigned long ret = spawn_erlang_epmd(NULL,
					  NULL,
					  0,
					  NULL,
					  args,
    ...
    /* success iff (p == pid) and (status == 0) */
    return (p == pid? status : ERL_NO_DAEMON);
#endif
} /* erl_start_epmd() */
...
static struct in_addr *get_addr(const char *hostname, struct in_addr *oaddr)
{
  ...
  hp = erl_gethostbyname_r(hostname,&host,buf,1024,&herror);
#else
  hp = erl_gethostbyname(hostname);
#endif
