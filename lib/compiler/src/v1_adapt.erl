...
module({Mod,Exp,Forms0}, Opts) ->
    put(fun_count, 0),
    put(funs, []),
    {Forms,Attr} = forms(Forms0, [], []),
    erase(current_function),
    erase(funs),
    erase(fun_count),
    {ok,{Mod,Exp,Attr,Forms}}.

%% forms(Fs) -> lists:map(fun (F) -> form(F) end, Fs).

forms([F|Fs], Facc, Aacc) ->
    case form(F) of
	{attribute,L,A,V} ->
	    forms(Fs, Facc, [{A,V}|Aacc]);
	{function,Line,N,A,Cs}=Func ->
	    forms(Fs, [Func|Facc], Aacc)
    end;
forms([], Facc, Aacc) ->
    {lists:reverse(Facc, get(funs)),lists:reverse(Aacc)}.
...
form({function,Line,Name0,Arity0,Clauses0}) ->
    put(current_function, {Name0,Arity0}),
...
clause({clause,Line,H0,G0,B0}) ->
	...
	_  -> [{clause,Line,H1,Grd,B1} || Grd <- G1]
    end.
...
guard_test({call,Line,{remote,Rline,{atom,Lm,erlang},{atom,La,F}},As0}) ->
    case erl_internal:type_test(F, length(As0)) of
	true -> As1 = gexpr_list(As0),
		{call,Line,{atom,La,F},As1}
    end;
...
gexpr({call,Line,{remote,Lr,{atom,Lm,erlang},{atom,La,F}},As0}) ->
    case erl_internal:guard_bif(F, length(As0)) of
	true -> As1 = gexpr_list(As0),
		{call,Line,{remote,Lr,{atom,Lm,erlang},{atom,La,F}},As1}
    end;
gexpr({op,Line,Op,A0}) ->
...
expr({'fun',Line,{clauses,Cs0},{Uniq,Hvss,Free0}}) ->
    I = get(fun_count),
    put(fun_count, I+1),
    {F,A} = get(current_function),
    Name = new_fun_name(I, F, A),
    Cs = fun_clauses(Cs0, Hvss, Free0),
    [{clause,_,H,G,B}|_] = Cs,
    Arity = length(H),
    put(funs, [{function,0,Name,Arity,Cs}|get(funs)]),
    Free = [{var,0,Var} || Var <- Free0],
    {make_fun,Line,Free,{Name,Arity,I,Uniq}};
