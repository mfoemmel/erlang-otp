...
init() ->
    ...
    gtk:exec("toplevel .dfw;wm withdraw .dfw;" %deiconify
...
width_height(DB, Font, Txt) when list(Font) ->
    case gtk:call([".dfw.l co -font ", Font,
		   " -text ", gtk:to_ascii(Txt)]) of
	{result, _} ->
...
choose_ascii(DB, Font) ->
    ...
    {variable,V} =gtk_db:lookup(DB,{font,Fam,Styl,Siz}),
    V.
...
choose(DB, Fam, Styl, Siz) ->
    ...
    case font_exist(DB, Fam, Styl, Siz) of
	false -> 
	    case gtk_db:lookup(DB,{font,Fam, Styl, Siz}) of
		{replaced_by,{font,F,S1,S2}} -> {F,S1,S2};
		_ ->
		    {F,S1,S2} = choose_other(DB,Fam, Styl, Siz),
		    gtk_db:insert(DB,{font,Fam, Styl, Siz},
...
font_variable(DB,Fam,Style,Siz) ->
	...
	false -> false;
	{replaced_by,Font} ->
	    gtk_db:lookup(DB,Font)
    end.

%% Returns: false|{variable,V}|
font_exist(DB, Fam,Style,Siz) ->
    case gtk_db:lookup(DB,{font,Fam,Style,Siz}) of
	...
	undefined -> 
		...
		undefined -> false;
		WH ->
		    N = gtk_db:counter(DB,font),
		    Var=["f",gtk:to_ascii(N)],
		    gtk:exec(["global ",Var,";set ",Var," ",FontStr]),
		    ...
		    gtk_db:insert(DB,{font,Fam,Style,Siz},V2),
		    V2
	    end
    end.
...
mk_font(DB, screen,[], Sz) ->
    Str2 = lists:append("screen", gtk:to_ascii(Sz)),
...
mk_font(DB, screen,[bold], Sz) ->
    Str2 = lists:append(["screen", gtk:to_ascii(Sz),"b"]),
    case width_height(DB,Str2,"a") of
	undefined ->
	    %% not my problem
