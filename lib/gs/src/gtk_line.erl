...
option(Option, Gtkid, Canvas, DB, AItem) ->
    case Option of
	{arrow,       Where} -> {s, [" -arrow ", gtk:to_ascii(Where)]};
	{capstyle,    Style} -> {s, [" -ca ", gtk:to_ascii(Style)]};
	{fg,          Color} -> {s, [" -f ", gtk:to_color(Color)]};
	{joinstyle,   Style} -> {s, [" -jo ", gtk:to_ascii(Style)]};
	{smooth,       Bool} -> {s, [" -sm ", gtk:to_ascii(Bool)]};
	{splinesteps,   Int} -> {s, [" -sp ", gtk:to_ascii(Int)]};
	{width,         Int} -> {s, [" -w ", gtk:to_ascii(Int)]};
