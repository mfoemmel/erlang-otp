    ...
    char **argv;		/* Argument strings. */
{
    ...
    Itk_SetWidgetCommand(entryPtr->tkwin, entryPtr->widgetCmd);
    entryPtr->numChars = 0;
    ...
    char *memPtr;		/* Info about entry widget. */
{
	...
	Tcl_UntraceVar(entryPtr->interp, entryPtr->textVarName,
		...
		(Tcl_VarTraceProc *) EntryTextVarProc, (ClientData) entryPtr);
    }
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (entryPtr->tkwin != NULL) {
	    Itk_SetWidgetCommand(entryPtr->tkwin, (Tcl_Command)NULL);
	    entryPtr->tkwin = NULL;
	    Tcl_DeleteCommand2(entryPtr->interp, entryPtr->widgetCmd);
	}
    ...
    int flags;			/* Information about what happened. */
{
	...
	if ((flags & TCL_TRACE_DESTROYED) && !(flags & TCL_INTERP_DESTROYED)) {
	    ...
	    Tcl_TraceVar(interp, entryPtr->textVarName,
		    TCL_GLOBAL_ONLY|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
		    (Tcl_VarTraceProc *) EntryTextVarProc, clientData);
	}
	return (char *) NULL;
    }
