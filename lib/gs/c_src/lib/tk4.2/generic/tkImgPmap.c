    ...
    PixmapInstance *instancePtr;
{
    ...
    TkpXpmAllocTmpBuffer(masterPtr, instancePtr, &image, &mask);
	...
	for (j=0; j<masterPtr->size[0]; j++) {
		...
		for (k=0; k<masterPtr->ncolors; k++) {
		    if (*p == colors[k].c) {
			TkpXpmSetPixel(instancePtr, image, mask, j, i,
    ...
    TkpXpmRealizePixmap(masterPtr, instancePtr, image, mask, isTransp);
    TkpXpmFreeTmpBuffer(masterPtr, instancePtr, image, mask);
}

/*
 *----------------------------------------------------------------------
    ...
    PixmapInstance *instancePtr;	/* Instance to reconfigure. */
{
    ...
    TkpXpmFreeInstanceData(instancePtr, 0);
				 ...
				 * image. */
{
    ...
    TkpInitPixmapInstance(masterPtr, instancePtr);
    ImgXpmConfigureInstance(instancePtr);
...
{
    TkpXpmDisplay(clientData, display, drawable, imageX, imageY, width,
	height, drawableX, drawableY);
}

/*
    ...
    Display *display;		/* Display containing window that used image.*/
{
    ...
    TkpXpmFreeInstanceData(instancePtr, 1);
				 ...
				 * image.  Must not have any more instances. */
{
	...
	Tcl_DeleteCommand2(masterPtr->interp, masterPtr->imageCmd);
    }
