					 ...
					 * to execute commands. */
{
     ...
     * are no windows left, Tcl_MainLoop returns and we exit.
     */
    ...
    int mask;				/* Not used. */
{
    ...
    if (newchan != chan) {
	Tcl_DeleteChannelHandler(chan, StdinProc, (ClientData) newchan);
    }
    if (newchan) {
	Tcl_CreateChannelHandler(newchan, TCL_READABLE, StdinProc,
		(ClientData) newchan);
    }
