    ...
    char **argv;		/* Argument strings. */
{
    ...
    scalePtr->toValue = 0;
    Itk_SetWidgetCommand(scalePtr->tkwin, scalePtr->widgetCmd);
    ...
    char *memPtr;	/* Info about scale widget. */
{
	...
	Tcl_UntraceVar(scalePtr->interp, scalePtr->varName,
		...
		(Tcl_VarTraceProc *) ScaleVarProc, (ClientData) scalePtr);
    }
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (scalePtr->tkwin != NULL) {
	    Itk_SetWidgetCommand(scalePtr->tkwin, (Tcl_Command)NULL);
	    scalePtr->tkwin = NULL;
	    Tcl_DeleteCommand2(scalePtr->interp, scalePtr->widgetCmd);
	}
    ...
    int flags;			/* Information about what happened. */
{
	...
	if ((flags & TCL_TRACE_DESTROYED) && !(flags & TCL_INTERP_DESTROYED)) {
	    Tcl_TraceVar(interp, scalePtr->varName,
		    TCL_GLOBAL_ONLY|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
		    (Tcl_VarTraceProc *) ScaleVarProc, clientData);
	    scalePtr->flags |= NEVER_SET;
