    ...
    char **argv;		/* Argument strings. */
{
    ...
    Itk_SetWidgetCommand(listPtr->tkwin, listPtr->widgetCmd);
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (listPtr->tkwin != NULL) {
	    ...
	    Itk_SetWidgetCommand(listPtr->tkwin, (Tcl_Command)NULL);
	    listPtr->tkwin = NULL;
	    Tcl_DeleteCommand2(listPtr->interp, listPtr->widgetCmd);
	}
...
static void
ListboxSelect(listPtr, first, last, select_flag)
    register Listbox *listPtr;		/* Information about widget. */
    int first;				/* Index of first element to
					 * select or deselect. */
    int last;				/* Index of last element to
					 * select or deselect. */
    int select_flag;			/* 1 means select items, 0 means
					 * deselect them. */
{
    ...
    firstRedisplay = -1;
    increment = select_flag ? 1 : -1;
    for (i = 0, elPtr = listPtr->firstPtr; i < first;
	    i++, elPtr = elPtr->nextPtr) {
	/* Empty loop body. */
    }
    for ( ; i <= last; i++, elPtr = elPtr->nextPtr) {
	if (elPtr->selected == select_flag) {
	    continue;
	}
	listPtr->numSelected += increment;
	elPtr->selected = select_flag;
	if (firstRedisplay < 0) {
	    firstRedisplay = i;
	}
	lastRedisplay = i;
