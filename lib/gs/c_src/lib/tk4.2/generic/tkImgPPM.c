				 ...
				 * in image being read. */
{
    ...
    if (type == PGM) {
        block.pub.pixelSize = 1;
	block.pub.offset[1] = 0;
	block.pub.offset[2] = 0;
    }
    else {
        block.pub.pixelSize = 3;
	block.pub.offset[1] = 1;
	block.pub.offset[2] = 2;
    }
    block.pub.offset[0] = 0;
    block.pub.offset[3] = 0;
    ...
    if (srcY > 0) {
	fseek(f, (long) (srcY * block.pub.pitch), SEEK_CUR);
    }

    nLines = (MAX_MEMORY + block.pub.pitch - 1) / block.pub.pitch;
    if (nLines > height) {
	nLines = height;
    }
    if (nLines <= 0) {
    ...
    nBytes = nLines * block.pub.pitch;
    pixelPtr = (unsigned char *) ckalloc((unsigned) nBytes);
    block.pub.pixelPtr = pixelPtr + srcX * block.pub.pixelSize;
	...
	block.pub.height = nLines;
	Tk_PhotoPutBlock(imageHandle, &block.pub, destX, destY, width, nLines);
	destY += nLines;
    }
