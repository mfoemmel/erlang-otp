...
static int		SquareCmd _ANSI_ARGS_((ClientData clientData,
...
#endif
{
    ...
    Itk_SetWidgetCommand(squarePtr->tkwin, squarePtr->widgetCmd);
    squarePtr->x = 0;
    squarePtr->y = 0;
    squarePtr->size = 20;
...
#endif
{
	...
	if (squarePtr->tkwin != NULL) {
	    Itk_SetWidgetCommand(squarePtr->tkwin, (Tcl_Command)NULL);
	    squarePtr->tkwin = NULL;
	    Tcl_DeleteCommand2(squarePtr->interp, squarePtr->widgetCmd);
	}
	if (squarePtr->updatePending) {
