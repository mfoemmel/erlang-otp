...
Tk_CreateImageType(typePtr)
    Tk_ImageType *typePtr;	/* Structure describing the type.  All of
				 ...
				 * in by caller. */
{
    Tk_ImageType *typePtr2,*nextPtr;

    typePtr2 = imageTypeList;
    while (typePtr2 && strcmp(typePtr2->name,typePtr->name)) {
	typePtr2 = typePtr2->nextPtr;
    }
    if (typePtr2) {
	nextPtr = typePtr2->nextPtr;
	*typePtr2 = *typePtr;
	typePtr2->nextPtr = nextPtr;
    } else {
    ...
    ClientData clientData;	/* One-word argument to pass to damageProc. */
{
    ...
    hPtr = Tcl_FindHashEntry(&((TkWindow *) tkwin)->mainPtr->imageTable, (char *) name);
    if (hPtr == NULL) {
	goto noSuchImage;
    }
    masterPtr = (ImageMaster *) Tcl_GetHashValue(hPtr);
