    ...
    char **argv;		/* Argument strings. */
{
    ...
    Itk_SetWidgetCommand(canvasPtr->tkwin, canvasPtr->widgetCmd);
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (canvasPtr->tkwin != NULL) {
	    Itk_SetWidgetCommand(canvasPtr->tkwin, (Tcl_Command)NULL);
	    canvasPtr->tkwin = NULL;
	    Tcl_DeleteCommand2(canvasPtr->interp, canvasPtr->widgetCmd);
	}
