...
static TkCmd commands[] = {
    ...
    {"bell",		Tk_BellCmd,		0},
    {"bind",		Tk_BindCmd,		1},
    {"bindtags",	Tk_BindtagsCmd,		1},
    {"clipboard",	Tk_ClipboardCmd,	0},
    {"destroy",		Tk_DestroyCmd,		1},
    {"event",		Tk_EventCmd,		1},
    {"focus",		Tk_FocusCmd,		1},
    {"grab",		Tk_GrabCmd,		0},
    {"grid",		Tk_GridCmd,		1},
    {"image",		Tk_ImageCmd,		1},
    {"lower",		Tk_LowerCmd,		1},
    {"option",		Tk_OptionCmd,		1},
    {"pack",		Tk_PackCmd,		1},
    {"place",		Tk_PlaceCmd,		1},
    {"raise",		Tk_RaiseCmd,		1},
    {"selection",	Tk_SelectionCmd,	0},
    {"tk",		Tk_TkCmd,		0},
    {"tkwait",		Tk_TkwaitCmd,		0},
    {"tk_chooseColor",  Tk_ChooseColorCmd,	0},
    {"tk_getOpenFile",  Tk_GetOpenFileCmd,	0},
    {"tk_getSaveFile",  Tk_GetSaveFileCmd,	0},
    {"tk_messageBox",   Tk_MessageBoxCmd,	0},
    {"winfo",		Tk_WinfoCmd,		1},
    {"wm",		Tk_WmCmd,		0},
     ...
     */
    {"button",		Tk_ButtonCmd,		1},
    {"canvas",		Tk_CanvasCmd,		1},
    {"checkbutton",	Tk_CheckbuttonCmd,	1},
    {"entry",		Tk_EntryCmd,		1},
    {"frame",		Tk_FrameCmd,		1},
    {"label",		Tk_LabelCmd,		1},
    {"listbox",		Tk_ListboxCmd,		1},
    {"menu",		Tk_MenuCmd,		0},
    {"menubutton",	Tk_MenubuttonCmd,	1},
    {"message",		Tk_MessageCmd,		1},
    {"radiobutton",	Tk_RadiobuttonCmd,	1},
    {"scale",		Tk_ScaleCmd,		1},
    {"scrollbar",	Tk_ScrollbarCmd,	1},
    {"text",		Tk_TextCmd,		1},
    {"toplevel",	Tk_ToplevelCmd,		0},
    /*
     * Native widget class commands.
     */
#ifdef MAC_TCL
    {"macscrollbar",	Tk_MacScrollbarCmd,	1},
    {"unsupported1",	TkUnsupported1Cmd,	1},
#endif
...
static Tk_Window	CreateTopLevelWindow _ANSI_ARGS_((Tcl_Interp *interp,
			    Tk_Window parent, char *wname, char *screenName));
...
static Tk_Window
CreateTopLevelWindow(interp, parent, wname, screenName)
    Tcl_Interp *interp;		/* Interpreter to use for error reporting. */
    Tk_Window parent;		/* Token for logical parent of new window
				 * (used for naming, options, etc.).  May
				 * be NULL. */
    char *wname;		/* Name for new window;  if parent is
				 ...
				 * parent. */
{
...
#if !defined(MAC_TCL) || defined(ITCL_NAMESPACES)
	...
	Tk_CreatePhotoImageFormat(&tkImgFmtXPM);
#endif /* MAC_TCL */
    ...
    if (parent != NULL) {
	if (NameWindow(interp, winPtr, (TkWindow *) parent, wname) != TCL_OK) {
...
#define FIXED_SIZE 200
    ...
    winPtr->nameUid = Tk_GetUid(wname);
    ...
    if (length1 == 1) {
	pathName[0] = '.';
	strcpy(pathName+1, wname);
    } else {
	strcpy(pathName, parentPtr->pathName);
	pathName[length1] = '.';
	strcpy(pathName+length1+1, wname);
    }
    ...
    if (!new) {
	Tcl_AppendResult(interp, "window name \"", wname,
    ...
    Tk_Window tkwin;		/* Window to destroy. */
{
	...
	if (winPtr->mainPtr->refCount == 0) {
...
#ifdef ITCL_NAMESPACES
		nsToken = Itcl_ActivateNamesp(winPtr->mainPtr->interp,
		    Itcl_GetGlobalNamesp(winPtr->mainPtr->interp));
#endif
                for (cmdPtr = commands; cmdPtr->name != NULL; cmdPtr++) {
		...
		Itcl_DeactivateNamesp(winPtr->mainPtr->interp, nsToken);
#else
                Tcl_UnlinkVar(winPtr->mainPtr->interp, "tk_strictMotif");
#endif
            }
 ...
 *	invokes the Tk_Init procedure that does the real work.
 *
 * Results:
 *	Returns a standard Tcl completion code and sets interp->result
 *	if there is an error.
...
/*
 * ------------------------------------------------------------------------
 *  Itk_SetWidgetCommand()
 *
 *  Used to associate a widget access command with a Tk window.  Widgets
 *  like the button, entry, etc., first create a window, then create
 *  an access command by calling Tcl_CreateCommand(), and then associate
 *  the two by calling this procedure.  This information can later be
 *  queried back vi Itk_GetWidgetCommand().
 * ------------------------------------------------------------------------
 */
void
Itk_SetWidgetCommand(tkwin,cmd)
    Tk_Window tkwin;      /* window representing widget */
    Tcl_Command cmd;      /* access command for widget */
{
    TkWindow *winPtr = (TkWindow*)tkwin;
    winPtr->widgetCmd = cmd;
}
 ...
 *  Itk_SetMegaWidget()
 *
 *  Used to associate a mega-widget container with a Tk window.  This
 *  is normally invoked by a mega-widget package (such as [incr Tk])
 *  whenever a new component widget is registered with a mega-widget
 *  container.  This information can later be queried back via
 *  Itk_GetMegaWidget().
 * ------------------------------------------------------------------------
 */
void
Itk_SetMegaWidget(tkwin,megawin)
    Tk_Window tkwin;      /* window representing widget */
    Tk_Window megawin;    /* mega-widget containing this widget */
{
    TkWindow *winPtr = (TkWindow*)tkwin;
    winPtr->megaPtr = (TkWindow*)megawin;
}

/*
 *----------------------------------------------------------------------
 ...
 *	This function is identical to Tcl_DeleteCommand2().
 *
 * Results:
...
#ifdef Tcl_DeleteCommand2
#   undef Tcl_DeleteCommand2
#endif

int
TkDeleteCommand2(interp, cmd)
    Tcl_Interp *interp;		/* Token for command interpreter (returned
				 * by a previous call to Tcl_CreateInterp). */
    Tcl_Command cmd;		/* Token for command to remove. */
{
#ifdef ITCL_NAMESPACES
    return Tcl_DeleteCommand2(interp, cmd);
#else
...
#ifdef Tcl_GetCommandPath
#   undef Tcl_GetCommandPath
#endif

char *
TkGetCommandPath(interp, cmd)
    Tcl_Interp *interp;		/* Token for command interpreter (returned
				 * by a previous call to Tcl_CreateInterp). */
    Tcl_Command cmd;		/* Token for command to remove. */
{
#ifdef ITCL_NAMESPACES
    return Tcl_GetCommandPath(interp, cmd);
#else
    return Tcl_GetCommandName(interp, cmd);
#endif
}
