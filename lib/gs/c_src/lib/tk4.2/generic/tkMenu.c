    ...
    char **argv;		/* Argument strings. */
{
    ...
    Itk_SetWidgetCommand(menuPtr->tkwin, menuPtr->widgetCmd);
    ...
    char *memPtr;		/* Pointer to entry to be freed. */
{
	...
	Tcl_UntraceVar(menuPtr->interp, mePtr->name,
		...
		(Tcl_VarTraceProc *) MenuVarProc, (ClientData) mePtr);
    }
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (menuPtr->tkwin != NULL) {
	    Itk_SetWidgetCommand(menuPtr->tkwin, (Tcl_Command)NULL);
	    menuPtr->tkwin = NULL;
	    Tcl_DeleteCommand2(menuPtr->interp, menuPtr->widgetCmd);
	}
    ...
    int flags;			/* Describes what just happened. */
{
	...
	if ((flags & TCL_TRACE_DESTROYED) && !(flags & TCL_INTERP_DESTROYED)) {
	    Tcl_TraceVar(interp, mePtr->name,
		    TCL_GLOBAL_ONLY|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
		    (Tcl_VarTraceProc *) MenuVarProc, clientData);
	}
	EventuallyRedrawMenu(menuPtr, (MenuEntry *) NULL);
	return (char *) NULL;
    }
