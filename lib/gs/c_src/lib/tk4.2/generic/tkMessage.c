    ...
    char **argv;		/* Argument strings. */
{
    ...
    Itk_SetWidgetCommand(msgPtr->tkwin, msgPtr->widgetCmd);
    msgPtr->string = NULL;
    msgPtr->numChars = 0;
    ...
    char *memPtr;		/* Info about message widget. */
{
	...
	Tcl_UntraceVar(msgPtr->interp, msgPtr->textVarName,
		...
		(Tcl_VarTraceProc *) MessageTextVarProc, (ClientData) msgPtr);
    }
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (msgPtr->tkwin != NULL) {
	    Itk_SetWidgetCommand(msgPtr->tkwin, (Tcl_Command)NULL);
	    msgPtr->tkwin = NULL;
	    Tcl_DeleteCommand2(msgPtr->interp, msgPtr->widgetCmd);
	}
    ...
    int flags;			/* Information about what happened. */
{
	...
	if ((flags & TCL_TRACE_DESTROYED) && !(flags & TCL_INTERP_DESTROYED)) {
	    ...
	    Tcl_TraceVar(interp, msgPtr->textVarName,
		    TCL_GLOBAL_ONLY|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
		    (Tcl_VarTraceProc *) MessageTextVarProc, clientData);
	}
	return (char *) NULL;
    }
