    ...
    char **argv;		/* Argument strings. */
{
    ...
    Itk_SetWidgetCommand(mbPtr->tkwin, mbPtr->widgetCmd);
    mbPtr->menuName = NULL;
    mbPtr->text = NULL;
    mbPtr->numChars = 0;
    mbPtr->underline = -1;
    ...
    char *memPtr;		/* Info about button widget. */
{
	...
	Tcl_UntraceVar(mbPtr->interp, mbPtr->textVarName,
		...
		(Tcl_VarTraceProc *) MenuButtonTextVarProc, (ClientData) mbPtr);
    }
    if (mbPtr->image != NULL) {
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if (mbPtr->tkwin != NULL) {
	    Itk_SetWidgetCommand(mbPtr->tkwin, (Tcl_Command)NULL);
	    mbPtr->tkwin = NULL;
	    Tcl_DeleteCommand2(mbPtr->interp, mbPtr->widgetCmd);
	}
    ...
    int flags;			/* Information about what happened. */
{
	...
	if ((flags & TCL_TRACE_DESTROYED) && !(flags & TCL_INTERP_DESTROYED)) {
	    ...
	    Tcl_TraceVar(interp, mbPtr->textVarName,
		    TCL_GLOBAL_ONLY|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
		    (Tcl_VarTraceProc *) MenuButtonTextVarProc, clientData);
	}
	return (char *) NULL;
    }
