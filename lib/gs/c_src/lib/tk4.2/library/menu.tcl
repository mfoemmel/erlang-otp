...
bind Menubutton <space> {
    tkMbPost %W
    tkMenuFirstEntry [[winfo_command %W] cget -menu]
}
...
proc tkMenuUnpost menu {
	...
	if {$mb != ""} {
	    set qmb [winfo_command $mb]
	    set menu [$qmb cget -menu]
	    [winfo_command $menu] unpost
	    set tkPriv(postedMb) {}
		...
		[winfo_command $parent] activate none
		[winfo_command $parent] postcascade none
...
proc tkMbMotion {w upDown rootx rooty} {
	...
	if {($new != "") && ([winfo class $new] == "Menubutton")
		&& ([[winfo_command $new] cget -indicatoron] == 0)
		&& ([[winfo_command $w] cget -indicatoron] == 0)} {
	    if {$upDown == "down"} {
...
proc tkMbButtonUp w {
	...
	tkMenuFirstEntry [[winfo_command $tkPriv(postedMb)] cget -menu]
    } else {
	tkMenuUnpost {}
    }
}
...
proc tkMenuButtonDown menu {
    global tkPriv
    [winfo_command $menu] postcascade active
    if {$tkPriv(postedMb) != ""} {
...
proc tkMenuFind {w char} {
	...
	switch [winfo class $child] {
	    Menubutton {
		set qchild [winfo_command $child]
