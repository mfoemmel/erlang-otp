...
bind Text <1> {
    tkTextButton1 %W %x %y
    [winfo_command %W] tag remove sel 0.0 end
}
bind Text <B1-Motion> {
    set tkPriv(x) %x
    set tkPriv(y) %y
...
bind Text <Double-1> {
    ...
    tkTextSelectTo %W %x %y
    catch {[winfo_command %W] mark set insert sel.first}
}
bind Text <Triple-1> {
...
bind Text <Control-1> {
    [winfo_command %W] mark set insert @%x,%y
}
...
bind Text <Shift-Left> {
    tkTextKeySelect %W [[winfo_command %W] index {insert - 1c}]
}
...
bind Text <Control-Left> {
    tkTextSetCursor %W [[winfo_command %W] index {insert - 1c wordstart}]
}
bind Text <Control-Right> {
    tkTextSetCursor %W [[winfo_command %W] index {insert wordend}]
}
...
bind Text <Shift-Control-Left> {
    tkTextKeySelect %W [[winfo_command %W] index {insert - 1c wordstart}]
}
bind Text <Shift-Control-Right> {
    tkTextKeySelect %W [[winfo_command %W] index {insert wordend}]
}
...
bind Text <Control-Prior> {
    [winfo_command %W] xview scroll -1 page
}
...
bind Text <Delete> {
    if {[[winfo_command %W] tag nextrange sel 1.0 end] != ""} {
	[winfo_command %W] delete sel.first sel.last
    } else {
	[winfo_command %W] delete insert
	[winfo_command %W] see insert
    }
}
bind Text <BackSpace> {
    if {[[winfo_command %W] tag nextrange sel 1.0 end] != ""} {
	[winfo_command %W] delete sel.first sel.last
    } elseif [[winfo_command %W] compare insert != 1.0] {
	[winfo_command %W] delete insert-1c
	[winfo_command %W] see insert
    }
}

bind Text <Control-space> {
    [winfo_command %W] mark set anchor insert
}
bind Text <Select> {
    [winfo_command %W] mark set anchor insert
}
...
bind Text <Control-slash> {
    [winfo_command %W] tag add sel 1.0 end
}
bind Text <Control-backslash> {
    [winfo_command %W] tag remove sel 1.0 end
}
bind Text <<Cut>> {
    tk_textCut %W
}
...
bind Text <Control-k> {
    if !$tk_strictMotif {
	if [[winfo_command %W] compare insert == {insert lineend}] {
	    [winfo_command %W] delete insert
	} else {
	    [winfo_command %W] delete insert {insert lineend}
	}
    }
}
bind Text <Control-n> {
...
bind Text <Control-o> {
    if !$tk_strictMotif {
	[winfo_command %W] insert insert \n
	[winfo_command %W] mark set insert insert-1c
    }
}
bind Text <Control-p> {
    if !$tk_strictMotif {
...
bind Text <Meta-BackSpace> {
    if !$tk_strictMotif {
	[winfo_command %W] delete {insert -1c wordstart} insert
    }
}
...
bind Text <Control-h> {
    if !$tk_strictMotif {
	if [[winfo_command %W] compare insert != 1.0] {
	    [winfo_command %W] delete insert-1c
	    [winfo_command %W] see insert
	}
    }
...
bind Text <B2-Motion> {
	...
	if $tkPriv(mouseMoved) {
	    [winfo_command %W] scan dragto %x %y
	}
    }
}
set tkPriv(prevPos) {}
