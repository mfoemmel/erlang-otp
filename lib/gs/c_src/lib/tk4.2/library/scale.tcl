...
bind Scale <Enter> {
    if $tk_strictMotif {
	set tkPriv(activeBg) [[winfo_command %W] cget -activebackground]
	[winfo_command %W] config -activebackground [[winfo_command %W] cget -background]
    }
...
bind Scale <Leave> {
    if $tk_strictMotif {
	[winfo_command %W] config -activebackground $tkPriv(activeBg)
    }
    if {[[winfo_command %W] cget -state] == "active"} {
	[winfo_command %W] configure -state normal
    }
}
bind Scale <1> {
...
bind Scale <Home> {
    [winfo_command %W] set [[winfo_command %W] cget -from]
}
bind Scale <End> {
    [winfo_command %W] set [[winfo_command %W] cget -to]
}
