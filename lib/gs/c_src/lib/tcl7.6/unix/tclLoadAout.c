...
VOID * TclLoadDictionary_Tcl _ANSI_ARGS_((char *symbol));
 ...
 *----------------------------------------------------------------------
 *
 * TclLoadDictionary_Tcl --
 *
 * Search for functions in Tcl library.
 *
 * Results:
 *	addres of symbol, or NULL if it cannot be found.
 *	Currently, only the Tcl_Init() function can be found.
 *
 * Side effects:
 *	None.
 *
...
static struct {
  char * name;
  int (*value) _ANSI_ARGS_((Tcl_Interp *));
} dictionary[] = {
  { "Tcl_Init", Tcl_Init },
  { 0, 0 }
};
    ...
    int flags;
{
  ...
  if (Tcl_GetPathType(path) == TCL_PATH_RELATIVE) {
	...
	if (!q && (access(path, F_OK) == -1)) {
	    Tcl_DStringAppend(&fullPath, LIB_RUNTIME_DIR, -1);
	    Tcl_DStringAppend(&fullPath, "/", 1);
	    Tcl_DStringAppend(&fullPath, path, -1);
	    if (access(Tcl_DStringValue(&fullPath), F_OK) == -1) {
		Tcl_DStringSetLength(&fullPath, 0);
		Tcl_DStringAppend(&fullPath, "/usr/lib/", 9);
		Tcl_DStringAppend(&fullPath, path, -1);
		if (access(Tcl_DStringValue(&fullPath), F_OK) == -1) {
		    Tcl_DStringSetLength(&fullPath, 0);
		    Tcl_DStringAppend(&fullPath, "/lib/", 5);
		    Tcl_DStringAppend(&fullPath, path, -1);
		}
	    }
	}
  ...
  Tcl_DStringAppend (&linkCommandBuf, " -L", -1);
  Tcl_DStringAppend (&linkCommandBuf, LIB_RUNTIME_DIR, -1);
