...
static Var *		LookupVar _ANSI_ARGS_((Tcl_Interp *interp,
			    CONST char *part1, CONST char *part2, int flags,
			    CONST char *msg, int create, Var **arrayPtrPtr));
static int		MakeUpvar _ANSI_ARGS_((Interp *iPtr,
			    CallFrame *framePtr, CONST char *otherP1,
			    CONST char *otherP2, CONST char *myName,
			    int flags));
static ArraySearch *	ParseSearchId _ANSI_ARGS_((Tcl_Interp *interp,
			    Var *varPtr, CONST char *varName,
			    CONST char *string));
...
LookupVar(interp, part1, part2, flags, msg, create, arrayPtrPtr)
    ...
    CONST char *part1;		/* If part2 isn't NULL, this is the name
    ...
    CONST char *part2;		/* Name of an element within array, or NULL. */
				 ...
				 * this is set to NULL. */
{
    ...
    Var *varPtr;
    int new;
    CONST char *openParen, *closeParen;	/* If this procedure parses a name
    ...
    CONST char *elName;			/* Name of array element or NULL;
    ...
    *arrayPtrPtr = NULL;
    if ((flags & ITCL_GLOBAL_MASK) || (iPtr->varFramePtr == NULL)) {
...
Tcl_GetVar2(interp, part1, part2, flags)
    Tcl_Interp *interp;		/* Command interpreter in which variable is
				 * to be looked up. */
    CONST char *part1;		/* Name of array (if part2 is NULL) or
				 * name of variable. */
    CONST char *part2;		/* If non-null, gives name of element in
				 * array. */
    int flags;			/* OR-ed combination of TCL_GLOBAL_ONLY,
				 ...
				 * PART1_NOT_PARSED. */
{
	...
	p = (char *) ckalloc((unsigned) newSize);
	if (varPtr->valueSpace > 0) {
	    strcpy(p, varPtr->value.string);
	    ckfree(varPtr->value.string);
	}
	varPtr->valueSpace = newSize;
	varPtr->value.string = p;
    }
...
MakeUpvar(iPtr, framePtr, otherP1, otherP2, myName, flags)
    ...
    CONST char *otherP1, *otherP2;/* Two-part name of variable in framePtr. */
    CONST char *myName;		/* Name of variable in local table, which
...
Tcl_UpVar(interp, frameName, varName, localName, flags)
    ...
    CONST char *varName;	/* Name of a variable in interp.  May be
				 ...
				 * in an array. */
    CONST char *localName;	/* Destination variable name. */
...
Tcl_UpVar2(interp, frameName, part1, part2, localName, flags)
    ...
    CONST char *frameName;	/* Name of the frame containing the source
				 * variable, such as "1" or "#0". */
    CONST char *part1, *part2;	/* Two parts of source variable name. */
    CONST char *localName;	/* Destination variable name. */
...
CallTraces(iPtr, arrayPtr, varPtr, part1, part2, flags)
    ...
    Var *varPtr;			/* Variable whose traces are to be
					 * invoked. */
    CONST char *part1, *part2;		/* Variable's two-part name. */
					 ...
					 * to callbacks. */
{
	...
	for (tracePtr = arrayPtr->tracePtr;  tracePtr != NULL;
	    ...
	    result = (*tracePtr->traceProc)(tracePtr->clientData,
		    (Tcl_Interp *) iPtr, (char *) part1, (char *) part2, flags);
	    if (result != NULL) {
...
ParseSearchId(interp, varPtr, varName, string)
    ...
    CONST char *varName;	/* Name of array variable that search is
				 * supposed to be for. */
    CONST char *string;		/* String containing id of search.  Must have
				 ...
				 * name. */
{
    ...
    id = strtoul(string+2, (char **) &end, 10);
    if ((end == (string+2)) || (*end != '-')) {
	goto syntax;
    }
