    ...
    char **argv;		/* Argument strings. */
{
    ...
    } else if (strncmp(argv[1], "active", length) == 0) {
	char *argv1[3];
	argv1[0] = "after";
	argv1[1] = argv[2];
	argv1[2] = "active";
	return Tcl_FileEventCmd(clientData, interp, 3, argv1);
    ...
    } else if (strncmp(argv[1], "readable", length) == 0) {
	char *argv1[3];
	argv1[0] = "after";
	argv1[1] = argv[2];
	argv1[2] = "readable";
	return Tcl_FileEventCmd(clientData, interp, 3, argv1);
    } else if (strncmp(argv[1], "writable", length) == 0) {
	char *argv1[3];
	argv1[0] = "after";
	argv1[1] = argv[2];
	argv1[2] = "writable";
	...
	Tcl_AppendResult(interp, "bad argument \"", argv[1],
		"\": must be active, cancel, idle, info, readable, writable, or a number",
		(char *) NULL);
	return TCL_ERROR;
    }
    return TCL_OK;
				 ...
				 * wait forever. */
{
    ...
    while (1) {
	Tcl_WatchFile(file, mask | TCL_ACTIVE);
	if (timeout > 0) {
