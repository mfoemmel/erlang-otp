...
static int blockTimeSet = 0;	/* 0 means there is no maximum block
				 * time:  block forever. */
int tclActiveFlag = 0;		/* 1 means there is at least one
				 ...
				 * others defined by event sources. */
{
	...
	if ((Tcl_WaitForEvent(timePtr) == TCL_ERROR) && !tclActiveFlag) {
	    return 0;
	}
