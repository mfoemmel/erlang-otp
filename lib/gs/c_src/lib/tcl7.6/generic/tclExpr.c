...
/*
 *	Three functions to solve the DOMAIN error for shared libraries
 */

#ifndef TCL_NO_MATH

/*
 *	It should be possible to make the following 3 functions static.
 *	Only, with gcc on HP-UX-9 this results in errors. Pity.
 */
...
double tclAcos _ANSI_ARGS_((double value));
double tclAsin _ANSI_ARGS_((double value));
double tclSqrt _ANSI_ARGS_((double value));

double tclAcos(value)
double value;
{
...
static int		ExprParseString _ANSI_ARGS_((Tcl_Interp *interp,
			    CONST char *string, Value *valuePtr));
...
static BuiltinFunc funcTable[] = {
#ifndef TCL_NO_MATH
    {"acos", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) tclAcos},
    {"asin", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) tclAsin},
...
#ifndef NO_ERF
    {"erf", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) erf},
#ifndef NO_ERFC
    {"erfc", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) erfc},
#endif
#endif
...
#ifndef NO_LGAMMA
    {"lgamma", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) lgamma},
#endif
    {"hypot", 2, {TCL_DOUBLE, TCL_DOUBLE}, ExprBinaryFunc, (ClientData) hypot},
#ifndef NO_JN
    {"j0", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) j0},
    {"j1", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) j1},
    {"jn", 2, {TCL_INT, TCL_DOUBLE}, ExprBinaryFunc, (ClientData) jn},
#endif
    ...
    {"sqrt", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) tclSqrt},
    {"tan", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) tan},
    {"tanh", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) tanh},
#ifndef NO_YN
    {"y0", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) y0},
    {"y1", 1, {TCL_DOUBLE}, ExprUnaryFunc, (ClientData) y1},
    {"yn", 2, {TCL_INT, TCL_DOUBLE}, ExprBinaryFunc, (ClientData) yn},
#endif
#endif
				 ...
				 * Caller must have initialized pv field. */
{
	...
	if (ExprLooksLikeInt(string)) {
	    ...
	    if (*p == '-') {
		start = p+1;
		valuePtr->intValue = -((int)strtoul(start, (char **) &term, 0));
	    } else if (*p == '+') {
		start = p+1;
		valuePtr->intValue = strtoul(start, (char **) &term, 0);
	    } else {
		start = p;
		valuePtr->intValue = strtoul(start, (char **) &term, 0);
	    }
	    ...
	    errno = 0;
	    valuePtr->doubleValue = strtod(string, (char **) &term);
					 ...
					 * correctly. */
{
	...
	if (ExprLooksLikeInt(p)) {
	    errno = 0;
	    valuePtr->intValue = strtoul(p, (char **) &term, 0);
	    if (errno == ERANGE) {
		interp->result = "integer value too large to represent";
		Tcl_SetErrorCode(interp, "ARITH", "IOVERFLOW",
			interp->result, (char *) NULL);
	    ...
	    errno = 0;
	    valuePtr->doubleValue = strtod(p, (char **) &term);
	    if (term != p) {
		if (errno != 0) {
		    TclExprFloatError(interp, valuePtr->doubleValue);
		    return TCL_ERROR;
	...
	case '$':
	    ...
	    infoPtr->token = VALUE;
	    var = Tcl_ParseVar(interp, (char *) p, (char **) &infoPtr->expr);
	    if (var == NULL) {
		return TCL_ERROR;
	    }
	    Tcl_ResetResult(interp);
	...
	case '"':
	    infoPtr->token = VALUE;
	    result = TclParseQuotes(interp, (char *) infoPtr->expr, '"', 0,
		    (char **) &infoPtr->expr, &valuePtr->pv);
	    if (result != TCL_OK) {
		return result;
	    }
	    Tcl_ResetResult(interp);
	...
	case '{':
	    infoPtr->token = VALUE;
	    result = TclParseBraces(interp, (char *) infoPtr->expr,
		    (char **) &infoPtr->expr, &valuePtr->pv);
	    if (result != TCL_OK) {
		return result;
	    }
...
#ifndef NO_LGAMMA
	signgam = 0;	/* trick needed for AIX to
			 * force inclusion of this symbol */
#endif
    ...
    *((char *) p) = savedChar;
	...
	for (i = 0; ; i++) {
		...
		if (mathFuncPtr->argTypes[i] == TCL_INT) {
		    ...
		    if (((double)args[i].intValue) != (long) (valuePtr->doubleValue)) {
			interp->result =
			    "argument to math function didn't have integer value";
			return TCL_ERROR;
		    }
		} else {
		    args[i].type = TCL_DOUBLE;
    ...
    Tcl_Value *resultPtr;
{
	...
	resultPtr->doubleValue = (*func)(args[0].intValue, args[1].doubleValue);
    }
    if (errno != 0) {
	TclExprFloatError(interp, resultPtr->doubleValue);
	return TCL_ERROR;
