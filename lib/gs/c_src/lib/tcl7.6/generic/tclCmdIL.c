    ...
    char **argv;			/* Argument strings. */
{
	...
	return TCL_OK;
    } else if ((c == 'b') && (strncmp(argv[1], "byteorder", length) == 0)
	    && (length >= 2)) {
	if (argc != 2) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0],
		    " byteorder\"", (char *) NULL);
	...
	Tcl_AppendResult(interp, (Tcl_GetHostByteorder() != TCL_BIGENDIAN) ?
		"smallendian":"bigendian", (char *) NULL);
	return TCL_OK;
	...
	return TCL_OK;
    } else if ((c == 'r') && (strncmp(argv[1], "resource", length) == 0)) {
	char **value;

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    argv[0], " resource id\"", (char *) NULL);
	    return TCL_ERROR;
	}
	value = Tcl_GetStaticFile(argv[2]);
	if (value != (char **) NULL) {
	    char **p = value;
	    while (*p) {
	...
	Tcl_AppendResult(interp, "no entry \"",
		argv[2], "\" in resource", (char *) NULL);
	return TCL_ERROR;
	...
	Tcl_AppendResult(interp, "bad option \"", argv[1],
		"\": should be args, body, byteorder, cmdcount, commands, ",
		"complete, default, ",
		"exists, globals, hostname, level, library, loaded, locals, ",
		"nameofexecutable, patchlevel, procs, resource, script, ",
