				 ...
				 * stored. */
{
    ...
    if (!TclRegExec(winRootPatternPtr, (char *) path, (char *) path)) {
	return path;
    }
    ...
    CONST char *path;
{
	...
	case TCL_PLATFORM_MAC:
		...
		if (!TclRegExec(macRootPatternPtr, (char *) path, (char *) path)
    ...
    Tcl_DString *bufPtr;	/* Pointer to DString to use for the result. */
{
    ...
    if (TclRegExec(macRootPatternPtr, (char *) path, (char *) path) == 1) {
	/*
...
Tcl_TranslateFileName(interp, name, bufferPtr)
    ...
    CONST char *name;		/* File name, which may begin with "~"
