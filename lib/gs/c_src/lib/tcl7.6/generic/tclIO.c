...
typedef struct Channel {
    ...
    struct Channel* supercedes; /* Refers to channel this one was
				 * stacked upon */
} Channel;
...
Tcl_Write(chan, srcPtr, slen)
    ...
    CONST char *srcPtr;			/* Output to buffer. */
                                         ...
                                         * and we must compute its length. */
{
    ...
    char *dPtr;				/* Search variables for newline. */
    CONST char *sPtr;
    ...
    CONST char *newValue;	/* New value for option. */
{
    ...
    if (chanPtr->typePtr->setOptionProc != (Tcl_DriverSetOptionProc *) NULL) {
        return (chanPtr->typePtr->setOptionProc) (chanPtr->instanceData,
                interp, (char *) optionName, (char *) newValue);
    }
    
    if (interp != (Tcl_Interp *) NULL) {
        Tcl_AppendResult(interp, "bad option \"", optionName,
                "\": should be -blocking, -buffering, -buffersize, ",
                "-byteorder, -eofchar, -translation, ",
        ...
        if (chanPtr->interestMask & TCL_READABLE) {
            if ((!(chanPtr->flags & CHANNEL_BLOCKED)) &&
                ...
                (chanPtr->typePtr->watchChannelProc) (chanPtr->instanceData,
                        TCL_READABLE | (chanPtr->interestMask & TCL_ACTIVE));
            }
        }
        if ((chanPtr->interestMask & TCL_WRITABLE) &&
                (chanPtr->flags & TCL_WRITABLE)) {
            (chanPtr->typePtr->watchChannelProc) (chanPtr->instanceData,
                    TCL_WRITABLE | (chanPtr->interestMask & TCL_ACTIVE));
        }
        if ((chanPtr->interestMask & TCL_EXCEPTION) &&
                (chanPtr->flags & (TCL_READABLE | TCL_WRITABLE))) {
            (chanPtr->typePtr->watchChannelProc) (chanPtr->instanceData,
                    TCL_EXCEPTION | (chanPtr->interestMask & TCL_ACTIVE));
        }
    }
}

    ...
    char **argv;			/* Argument strings. */
{
    ...
    if ((c == 'r') && (strncmp(argv[2], "readable", length) == 0)) {
        mask = TCL_READABLE;
    } else if ((c == 'a') && (strncmp(argv[2], "active", length) == 0)) {
        mask = TCL_READABLE|TCL_ACTIVE;
    } else if ((c == 'w') && (strncmp(argv[2], "writable", length) == 0)) {
        mask = TCL_WRITABLE;
    } else {
	Tcl_AppendResult(interp, "bad event name \"", argv[2],
		"\": must be active, readable, or writable", (char *) NULL);
	return TCL_ERROR;
    }
    ...
    chanPtr = (Channel *) chan;
    if ((chanPtr->flags & (mask&(TCL_READABLE|TCL_WRITABLE))) == 0) {
