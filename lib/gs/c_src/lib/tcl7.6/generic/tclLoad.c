...
#include "tclInt.h"
#if defined(NO_DLFCN_H) || defined(__WIN32__)
...
typedef struct LoadedPackage {
    ...
    char *packageName;		/* Name of package prefix for the package,
				 ...
				 * Malloc-ed. */
    VOID *packageHandle;	/* Handle for the package, which can be used
				   later by dlsym() to find symbols in it. */
    ...
    char **argv;			/* Argument strings. */
{
    ...
    Tcl_DString pkgName, initName, fileName;
	...
	if (gotPkgName) {
		...
		for (p = pkgGuess; (*p) && (*p != '.'); p++) {
		    /* Empty loop body. */
		}
		if ((p>pkgGuess+3) && !strncmp(p-3,"_G0.",4)) {
		    p-=3;
		}
		while ((p-- > pkgGuess) && isdigit(UCHAR(*p))) {
		    /* Empty loop body. */
		}
		p++;
		if (p == pkgGuess) {
	...
	if (Tcl_GetPathType(argv[1]) == TCL_PATH_RELATIVE) {
	    p = TclGetCwd(interp);
	    Tcl_DStringSetLength(&fileName,0);
	    Tcl_DStringAppend(&fileName,p,-1);
	    Tcl_DStringAppend(&fileName,"/",-1);
	    Tcl_DStringAppend(&fileName,argv[1],-1);
	    fullFileName = Tcl_DStringValue(&fileName);
	}
#endif
	/*
	...
	handle = dlopen(fullFileName, RTLD_NOW | RTLD_GLOBAL);
	if (handle == (VOID *) NULL) {
	    Tcl_AppendResult(interp, "couldn't load file \"", fullFileName,
		    "\": ", dlerror(), (char *) NULL);
	    code = TCL_ERROR;
	    goto done;
	}
	...
	Tcl_DStringAppend(&initName, "_", 1);
	Tcl_DStringAppend(&initName, Tcl_DStringValue(&pkgName), -1);
	Tcl_DStringAppend(&initName, "_Init", 5);
	p = Tcl_DStringValue(&initName);
	initProc = (Tcl_PackageInitProc *) dlsym(handle, p+1);
	if (initProc == NULL) {
	    initProc = (Tcl_PackageInitProc *) dlsym(handle, p);
	    if (initProc == NULL) {
		Tcl_DStringAppend(&initName, "__FP10Tcl_Interp", 16);
		p = Tcl_DStringValue(&initName);	
		initProc = (Tcl_PackageInitProc *) dlsym(handle, p+1);
		if (initProc == NULL) {
		    initProc = (Tcl_PackageInitProc *) dlsym(handle, p);
	...
	if (initProc == NULL) {
	    Tcl_DStringSetLength(&initName,strlen(Tcl_DStringValue(&pkgName))+6);
	    p = Tcl_DStringValue(&initName);
	...
	Tcl_DStringSetLength(&initName,strlen(Tcl_DStringValue(&pkgName))+1);
	Tcl_DStringAppend(&initName, "_SafeInit", 9);

	p = Tcl_DStringValue(&initName);
	safeInitProc = (Tcl_PackageInitProc *) dlsym(handle, p+1);
	if (safeInitProc == NULL) {
	    safeInitProc = (Tcl_PackageInitProc *) dlsym(handle, p);
	    if (safeInitProc == NULL) {
		Tcl_DStringAppend(&initName, "__FP10Tcl_Interp", 16);
		p = Tcl_DStringValue(&initName);	
		safeInitProc = (Tcl_PackageInitProc *) dlsym(handle, p+1);
					 ...
					 * interpreters. */
{
    ...
    pkgPtr->packageName = (char *) ckalloc((unsigned)
	    (strlen(pkgName) + 1));
    pkgPtr->packageHandle = (VOID *) NULL;
...
TclGetLoadedPackages(interp, targetName)
    ...
    CONST char *targetName;	/* Name of target interpreter or NULL.
 ...
 *----------------------------------------------------------------------
 *
 * Tcl_FindPackageHandle --
 *
 *	This procedure returns the handle related to a loaded package.
 *
 * Results:
 *	The return value is a handle that later can be used
 *	by dlsym().
 *
 * Side effects:
 *	None.
...
VOID *
Tcl_FindPackageHandle(packageName)
    CONST char *packageName;	/* Name of package */
{
    LoadedPackage *pkgPtr;
    VOID *handle = (VOID *) NULL;
    static VOID *genericHandle = (VOID *) NULL;

    for (pkgPtr = firstPackagePtr; pkgPtr != NULL;
	    pkgPtr = pkgPtr->nextPtr) {
	if (!strcmp(pkgPtr->packageName,packageName)) {
	    handle = pkgPtr->packageHandle;
	}
    }
    if (handle == (VOID *) NULL) {
	if (genericHandle == NULL) {
	    genericHandle = dlopen ((char *) NULL, RTLD_NOW);
	}
