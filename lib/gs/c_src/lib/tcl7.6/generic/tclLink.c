				 ...
				 * OR'ed in. */
{
    ...
    if (Tcl_SetVar(interp, varName, StringValue(linkPtr, buffer),
	    ITCL_GLOBAL_VAR|TCL_LEAVE_ERR_MSG) == NULL) {
	ckfree(linkPtr->varName);
	ckfree((char *) linkPtr);
	return TCL_ERROR;
    }
    code = Tcl_TraceVar(interp, varName, ITCL_GLOBAL_VAR|TCL_TRACE_READS
    ...
    char *varName;		/* Global variable in interp to unlink. */
{
    ...
    Tcl_UntraceVar(interp, varName,
	    ITCL_GLOBAL_VAR|TCL_TRACE_READS|TCL_TRACE_WRITES|TCL_TRACE_UNSETS,
    ...
    int flags;			/* Miscellaneous additional information. */
{
	...
	if (flags & TCL_INTERP_DESTROYED) {
	    ...
	    Tcl_SetVar(interp, linkPtr->varName, StringValue(linkPtr, buffer),
		    ITCL_GLOBAL_VAR);
	    Tcl_TraceVar(interp, linkPtr->varName, ITCL_GLOBAL_VAR
    ...
    value = Tcl_GetVar(interp, linkPtr->varName, ITCL_GLOBAL_VAR);
    if (value == NULL) {
	/*
	...
	case TCL_LINK_INT:
		...
		Tcl_SetVar(interp, linkPtr->varName,
			StringValue(linkPtr, buffer), ITCL_GLOBAL_VAR);
