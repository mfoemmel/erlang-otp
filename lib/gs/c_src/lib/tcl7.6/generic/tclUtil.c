...
TclFindElement(interp, list, elementPtr, nextPtr, sizePtr, bracePtr)
    Tcl_Interp *interp;		/* Interpreter to use for error reporting. 
				 ...
				 * after errors. */
    CONST char *list;	/* String containing Tcl list with zero
				 ...
				 * in braces. */
{
	...
	*bracePtr = openBraces;
	...
	switch (*p) {
	    ...
	    case '\\': {
		(void) Tcl_Backslash((char *) p, &size);
		p += size - 1;
		break;
	    }
		...
		if (inQuotes) {
			...
			Tcl_ResetResult(interp);
			sprintf(interp->result,
				"list element in quotes followed by \"%.*s\" %s", (int) (p2-p), (char *) p,
				"instead of space");
		    }
		    return TCL_ERROR;
		}
				 ...
				 * of pointers to list elements. */
{
    ...
    argv = (char **) ckalloc((unsigned)
	    ((size * sizeof(char *)) + (q - list) + 1));
    for (i = 0, p = ((char *) argv) + size*sizeof(char *);
	    *list != 0; i++) {
	result = TclFindElement(interp, list, &element, (char **) &list,
		&elSize, &brace);
	if (result != TCL_OK) {
	    ckfree((char *) argv);
	    return result;
...
Tcl_SetResult(interp, string, freeProc)
    Tcl_Interp *interp;		/* Interpreter with which to associate the
				 * return value. */
    CONST char *string;		/* Value to be returned.  If NULL,
...
Tcl_SetErrorCode TCL_VARARGS_DEF(Tcl_Interp *,arg1)
{
    ...
    iPtr = (Interp *) TCL_VARARGS_START(Tcl_Interp *,arg1,argList);
    flags = ITCL_GLOBAL_VAR | TCL_LIST_ELEMENT;
    while (1) {
	string = va_arg(argList, char *);
	if (string == NULL) {
	    break;
...
TclGetListIndex(interp, string, indexPtr)
    ...
    CONST char *string;			/* String containing list index. */
				 ...
				 * string, so that "^" won't match. */
{
    ...
    TclRegError((char *) NULL);
    match = TclRegExec(regexpPtr, (char *) string, (char *) start);
    if (TclGetRegError() != NULL) {
	Tcl_ResetResult(interp);
    ...
    int flags;			/* Information about what happened. */
{
    ...
    prec = strtoul(value, (char **) &end, 10);
