...
catch {source -rsrc tcl:tclIndex}
...
# args -	Usually "args" is the name of a handling procedure.
#		But extra arguments can be specified when the handler
#		is registered, and these will be kept as arguments when
...
proc auto_load cmd {
    ...
    if [string compare [uplevel #0 info commands import] {}] {
	set command which
	foreach nsref [uplevel #0 import all] {
	...
	catch {uplevel #0 source -rsrc [string tolower [lindex $pkg 1]]:tclIndex}
    }
	...
	set error [catch {
		...
		foreach name [array names auto_index ::*] {
		    set auto_index([string range $name 2 end]) $auto_index($name)
...
proc auto_reset {} {
	...
	if {[info exists auto_index($p)] && ![string match auto_* $p]
		&& ([lsearch -exact {unknown pkg_mkIndex tclPkgSetup
			tclPkgUnknown tcl_unknown unknown_handler} $p] < 0)} {
	    rename $p {}
	}
    }
...
proc pkg_mkIndex {dir args} {
	...
	# If any package is loaded in the parent interpreter, load it into the
	# child also (if possible), in case the extension depends on it.

	foreach pkg [info loaded] {
	    $c eval {set argv {-geometry +0+0}}
	    catch {load [lindex $pkg 0] [lindex $pkg 1] $c}
	}
	...
	if [catch {
		...
		if {![string compare $ext ..o]} {
		    lappend ext .a
		}
		if {[lsearch -exact $ext [file extension $file]] >= 0} {
...
proc tclPkgUnknown {name version {exact {}}} {
    global auto_path tcl_platform env tcl_version

    if [catch {set dirlist $auto_path}] {
	set dirlist ""
    }
    for {set i [expr [llength $dirlist] - 1]} {$i >= 0} {incr i -1} {
	foreach file [glob -nocomplain [file join [lindex $dirlist $i] * pkgIndex.tcl]] {
    ...
    set dir [file join [file dirname [file dirname $dir]] "[string tolower $name]"]
