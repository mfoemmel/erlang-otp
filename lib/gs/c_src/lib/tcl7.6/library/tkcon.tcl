...
## tkConPluspatch - gives info about Pluspatches
##
proc tkConPluspatch {} {
  global tkCon tcl_version
  set page	"http://www.cogsci.kun.nl/tkpvm/pluspatch.html#about"
  set email	"nijtmans@nici.kun.nl"
  if [tk_dialog $tkCon(base).pluspatch "Help on Pluspatches v $tcl_version" \
	  "Plus-patches version $tcl_version\nwritten by:\nJan Nijtmans, $email\nHelp available at:\n$page" \
	  questhead 0 OK "Load into Netscape"] {
    update
    if {[catch {exec netscape -remote "openURL($page)"}]
	&& [catch {exec netscape $page &}]} {
...
proc tkConInitMenus {w title} {
  ...
  foreach m [list [menu $w.help.m] [menu $w.pop.help]] {
    $m add command -label "About Tkcon" -un 0 -acc Ctrl-A -command tkConAbout
    $m add command -label "About Pluspatches" -command tkConPluspatch
    $m add separator
