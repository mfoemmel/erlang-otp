...
get_op_def(Objkey, Op) ->
	...
	{_Local, _Key, _, _, Module} ->
	    case check_version(Module:module_info(attributes)) of
		true ->
		    case Module:oe_tc(Op) of
			undefined ->
		...
		false ->
		    case Module:handle_call({false, oe_get_interface, []}, false, false) of
			{reply, OpDef, _} ->
			    get_op_def_1(OpDef, atom_to_list(Op));
			_->
			    []
		    end
	    end;
