...
public abstract class AbstractConnection extends Thread {
  ...
  protected boolean cookieOk = false;  // already checked the cookie for this connection
  ...
  static {
    ...
    try {
      if (trace != null) defaultLevel = Integer.valueOf(trace).intValue();
    }
  ...
  protected synchronized void do_send(OtpOutputStream header, OtpOutputStream payload) 
    throws IOException {
    try {
      if (traceLevel >= sendThreshold) {
  ...
  protected int readSock(Socket s, byte[] b)
    throws IOException {
    int got = 0;
    int len = b.length;
    int i;
