...
send(S,{A,B,C,D,E,F,G,H},P,Data) when integer(A+B+C+D+E+F+G+H),P>=0,P=<16#ffff ->
    call(S, {sendto, {A,B,C,D,E,F,G,H}, P, Data}).
...
open_mgr(Owner, Tag, St) ->
    ...
    link(Owner),
    case inet:ll_open_set_bind(udp_inet,?INET_AF_INET6,St) of
...
socket_loop(Udp, Owner, St) ->
    receive
	{Udp, {data, [?INET_REP_DATA,P1,P0,
		      X1,X2,X3,X4,X5,X6,X7,X8,X9,X10,X11,X12,X13,X14,X15,X16 |
		      Data]}} ->
	    handle_input(St#sock.fs,
			 {inet:bytes_to_ip6(X1,X2,X3,X4,X5,X6,X7,X8,
...
ll_recv(Udp, Length,Time) ->
    case sync_cmd(Udp, 
		  ...
		  ?INET_REP_DATA) of
	{ok, [P1,P0, X1,X2,X3,X4,X5,X6,X7,X8,
	      X9,X10,X11,X12,X13,X14,X15,X16|Data]} ->
	    {ok, {inet:bytes_to_ip6(X1,X2,X3,X4,X5,X6,X7,X8,
				    X9,X10,X11,X12,X13,X14,X15,X16),
		  ?u16(P1,P0), Data}};
	Error ->
	    Error
...
ll_sendto(Udp,IP,Port,Data)  ->
    Udp ! {self(), {command, [?UDP_REQ_SENDTO, ?int16(Port),
			      inet:ip6_to_bytes(IP), Data]}},
    ok.
