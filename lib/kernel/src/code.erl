...
-spec(delete/1 :: (Module :: atom()) -> 'true' | 'false').
delete(Mod) when is_atom(Mod) -> call({delete,Mod}).

-spec(purge/1 :: (Module :: atom()) -> 'true' | 'false').
purge(Mod) when is_atom(Mod) -> call({purge,Mod}).

-spec(soft_purge/1 :: (Module :: atom()) -> 'true' | 'false').
soft_purge(Mod) when is_atom(Mod) -> call({soft_purge,Mod}).

-spec(is_loaded/1 :: (Module :: atom()) -> {'file', string() | loaded_ret_atoms()} | 'false' ).
is_loaded(Mod) when is_atom(Mod) -> call({is_loaded,Mod}).

-spec(get_object_code/1 :: (Module :: atom()) -> {atom(), binary(), string()} | 'error').
get_object_code(Mod) when is_atom(Mod) -> call({get_object_code, Mod}).

-spec(all_loaded/0 :: () -> [{atom(), string() | loaded_ret_atoms()}]).
all_loaded() -> call(all_loaded).

-spec(stop/0 :: () -> no_return()).
stop() -> call(stop).

-spec(root_dir/0 :: () -> string()).
...
-spec(replace_path/2 :: (Name:: atom(), Dir :: string()) -> 'true' | replace_path_error()).
...
-spec(which/1 :: (Module :: atom()) -> string() | which_ret_atoms()).
...
-spec(where_is_file/1 :: (Filename :: string()) -> string() | atom()).
