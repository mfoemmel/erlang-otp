...
%%important since some browsers include others
%%to behave as they were something else  
-define(MOZILLA_BROWSERS,[{opera,"opera"},{msie,"msie"}]).


%%If your operatingsystem is not recognized add it to this
%%list.
...
getBrowser(AgentString)->
	...
	{match,Start,Length}->
		...
		{mozilla,Vsn}->
		    {getMozilla(LAgentString,?MOZILLA_BROWSERS,{?NETSCAPE,Vsn}),operativeSystem(LAgentString)};
		AnyBrowser->
...
%%First we check if it is one of the browsers that not are the default mozillaborwser
%%against the regexp for the different browsers. if no match it a mozilla browser
%%i.e opera netscape or internet explorer
