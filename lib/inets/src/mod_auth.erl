...
-export([add_user/5, add_group_member/4, list_users/2, delete_user/3,
	 delete_group_member/4, list_groups/2, delete_group/3,
	 get_user/3, list_group_members/3]).
...
do_auth(Info, Directory, DirectoryData, AuthType) ->
    Port = httpd_util:lookup(Info#mod.config_db,port,80),
    %% Authenticate (require)
    case require(Info, Directory, DirectoryData, Port) of
	authorized ->
	    {proceed,Info#mod.data};
	{authorized, User} ->
...
require(Info, Directory, DirectoryData, Port) ->
	...
	_ ->
		...
		[$B,$a,$s,$i,$c,$ |EncodedString] ->
		    ...
		    case a_valid_user(Info, DecodedString, ValidUsers, ValidGroups, 
				      Directory, DirectoryData, Port) of
			{yes, User} ->
			    {authorized, User};
			{no, Reason} ->
			    {authorization_failed, Reason};
...
a_valid_user(Info, DecodedString, ValidUsers, ValidGroups, Dir, DirData, Port) ->
    case httpd_util:split(DecodedString,":",2) of
	{ok,[SupposedUser, Password]} ->
	    case user_accepted(SupposedUser, ValidUsers) of
		true ->
		    check_password(SupposedUser, Password, Dir, DirData, Port);
		false ->
		    case group_accepted(Info, SupposedUser, ValidGroups, Port, Dir, DirData) of
			true ->
			    check_password(SupposedUser, Password, Dir, DirData, Port);
			false ->
...
group_accepted(Info, User, undefined, Port, Dir, DirData) ->
    false;
group_accepted(Info, User, [], Port, Dir, DirData) ->
    false;
group_accepted(Info, User, [Group|Rest], Port, Dir, DirData) ->
    Ret = int_list_group_members(Group, Port, Dir, DirData),
    case Ret of
	{ok, UserList} ->
		...
		true ->
		    true;
		false ->
		    group_accepted(Info, User, Rest, Port, Dir, DirData)
	    end;
	Other ->
	    false
    end.

check_password(User, Password, Dir, DirData, Port) ->
...
int_list_group_members(Group, Port, Dir, DirData) ->
    AuthMod = auth_mod_name(DirData),
...
load([$A,$u,$t,$h,$M,$n,$e,$s,$i,$a,$D,$B,$ |AuthMnesiaDB],
	...
	"Off" ->
	    {ok,[{directory,Dir,[{auth_type,mnesia}|DirData]}|Rest]};
	_ ->
	    {error, ?NICE(http_conf:clean(AuthMnesiaDB)++" is an invalid AuthMnesiaDB")}
    end.
...
add_user(UserName, Password, UserData, Port, Dir) ->
    api_call(Port, Dir, add_user, [#httpd_user{username=UserName, 
						   password=Password,
						   user_data=UserData}]).
%% get_user
get_user(UserName, Port, Dir) ->
    api_call(Port, Dir, get_user, [UserName]).

%% add_group_member
add_group_member(GroupName, UserName, Port, Dir) ->
    api_call(Port, Dir, add_group_member, [GroupName, UserName]).

%% delete_group_member
delete_group_member(GroupName, UserName, Port, Dir) ->
    api_call(Port, Dir, delete_group_member, [GroupName, UserName]).
    
%% list_users
list_users(Port, Dir) ->
    api_call(Port, Dir, list_users, []).
    
%% delete_user
delete_user(UserName, Port, Dir) ->
    api_call(Port, Dir, delete_user, [UserName]).
    
%% delete_group
delete_group(GroupName, Port, Dir) ->
    api_call(Port, Dir, delete_group, [GroupName]).

%% list_groups
list_groups(Port, Dir) ->
    api_call(Port, Dir, list_groups, []).

%% list_group_members
list_group_members(GroupName, Port, Dir) ->
    api_call(Port, Dir, list_group_members, [GroupName]).

%% API gateway

api_call(Port, Dir, Func, Args) ->
    ConfigName = list_to_atom(lists:flatten(io_lib:format("httpd_conf~w",[Port]))),
