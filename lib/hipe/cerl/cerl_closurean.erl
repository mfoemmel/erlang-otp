...
%%% 	      [cerl_prettypr:format(StartFun, [noann])]),
    ...
    %% Gather a database of all fun-expressions in T and initialise all
    %% their outputs and parameter variables. Bind all module- and
...
visit(T, L, St) ->
	...
	apply ->
		...
		[X] ->
		    ...
		    Ls = set__to_list(X),
		    Out = St2#state.out,
		    Xs1 = join_list([dict:fetch(L, Out) || L <- Ls]),
