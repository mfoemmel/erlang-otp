...
map(Defun) ->
  ...
  %%hipe_amd64_cfg:pp(CFG0),
  Liveness = hipe_amd64_liveness:analyse(CFG0),
  StartLabel = hipe_amd64_cfg:start_label(CFG0),
  SuccMap = hipe_amd64_cfg:succ_map(CFG0),
...
do_blocks(Pred,[Lbl|Lbls], CFG, Liveness, Map, SuccMap, BlockMap) ->
  case gb_trees:lookup(Lbl, BlockMap) of
    none ->
      ...
      Block = hipe_amd64_cfg:bb(CFG, Lbl),
      Succ = hipe_amd64_cfg:succ(SuccMap, Lbl),
      NewBlockMap = gb_trees:insert(Lbl, Map, BlockMap),
      LiveOut = [X || X <- hipe_amd64_liveness:liveout(Liveness, Lbl),
		      is_fp(X)],
	...
	case Dirty of
	  true ->
	    ...
	    {hipe_amd64_cfg:bb_add(CFG, Lbl, NewBlock), SuccMap};
	  _ ->
	    {CFG, SuccMap}
	end,
    ...
    {value, ExistingMap}->
	 ...
	 true ->
	  ...
	  NewSuccMap = hipe_amd64_cfg:succ_map(NewCFG),
...
do_block([I|Is], LiveOut, Map, BlockMap, Dirty) ->
  case handle_insn(I) of
    ...
    true ->
      Def = ordsets:from_list(hipe_amd64_defuse:insn_def(I)),
      Use = ordsets:from_list(hipe_amd64_defuse:insn_use(I)),
      NewLiveOut = 
...
do_shuffle(Pred,Lbl,CFG, OldMap, NewMap)->
  ...
  %% Update the CFG.
  NewLabel = hipe_gensym:get_next_label(amd64),
  NewCFG1 = hipe_amd64_cfg:bb_add(CFG, NewLabel, hipe_bb:mk_bb(Code)),
  OldPred = hipe_amd64_cfg:bb(NewCFG1, Pred),
  ...
  hipe_amd64_cfg:bb_add(NewCFG1, Pred, NewPredBB).
...
do_switching([C|Cycles], Insns)->
  case length(C) of
    ...
    1 -> [X] = C,
	 NewInsns = Insns ++ [hipe_amd64:mk_fp_unop(fxch, mk_st(X))],
	 do_switching(Cycles, NewInsns);
    _ -> NewInsns = Insns ++ [hipe_amd64:mk_fp_unop(fxch, mk_st(X)) || X<-C],
	 do_switching(Cycles, NewInsns)
  end;
do_switching([],Insns) ->
  Insns.
...
redirect(Insn, OldLbl, NewLbl)->
  case Insn of
    #pseudo_call{contlab=ContLab, sdesc=SDesc} ->
      #amd64_sdesc{exnlab=ExnLab} = SDesc,
      if ContLab =:= OldLbl -> 
	  Insn#pseudo_call{contlab=NewLbl};
	 ExnLab =:= OldLbl ->
	  Insn#pseudo_call{sdesc=SDesc#amd64_sdesc{exnlab=NewLbl}}
      end;
    _ -> 
      hipe_amd64_cfg:redirect_jmp(Insn, OldLbl, NewLbl)
  end.
...
do_fmove(Src, Dst=#amd64_mem{},LiveOut, Map) ->
%%% Storing a float from the stack into memory.
  {SwitchInsn, NewMap0} = switch_first(Src, Map),
  case is_liveOut(Src, LiveOut) of
    true->
      {SwitchInsn ++[hipe_amd64:mk_fp_unop(fst, Dst)], NewMap0};
    _ ->
      NewMap1 = pop(NewMap0),
      {SwitchInsn ++[hipe_amd64:mk_fp_unop(fstp, Dst)], NewMap1}
  end;
do_fmove(Src=#amd64_mem{}, Dst, _LiveOut, Map) ->
...
%%% or converting a fixnum in a temp to a float on the fp stack.
  ...
  IsConv =
    case Src of
      #amd64_temp{type=Type}-> Type /= 'double';
      _ -> false
    end,
  case IsConv of
    true ->
...
do_conv(Src=#amd64_temp{reg=Reg},Dst,Map)->
  ...
  {Move, NewSrc} = 
	...
	{[hipe_amd64:mk_move(Src,Temp)],Temp};
      _ ->
	{[],Src}
    end,
  ...
  case length(PushOp) of
    1 -> %% No popping of memory object on fpstack
      {Move++[hipe_amd64:mk_fp_unop(fild, NewSrc)], NewMap};
    _ -> %% H contains pop instructions. Must be kept!
      Head = butlast(PushOp),
      {Move++Head++[hipe_amd64:mk_fp_unop(fild, NewSrc)], NewMap}
  end.
...
format_fp_binop(Op, Src=#amd64_temp{}, Dst=#amd64_fpreg{reg=Reg}) ->
  %% Handle that st(0) is sometimes implicit.
  if Reg==0-> [hipe_amd64:mk_fp_unop(Op, Src)];
     true-> [hipe_amd64:mk_fp_binop(Op, Src, Dst)]
  end;
format_fp_binop(Op, Src, Dst) ->
  [hipe_amd64:mk_fp_binop(Op, Src, Dst)].
...
push(X, Map0) ->
  ...
  NewMap = [X | Map],
  PushOp = [hipe_amd64:mk_fp_unop(fld, NewX)],
...
pop_insn() ->
  [hipe_amd64:mk_fp_unop('fstp',mk_st(0))].
...
pop_dead([D|Dead], Map, Code) ->
  ...
  NewMap = pop(NewMap0),
  Store = case D of
	    #amd64_temp{}->[hipe_amd64:mk_fp_unop('fstp', D)];
	    _ -> 
	      pop_insn()
	  end,
...
pop_member(Member, Map)->
  ...
  {[hipe_amd64:mk_fp_unop('fstp', mk_st(get_pos(Member, Map, 0)))],
   Head++T}.
...
pop_a_temp(Map) ->
  ...
  NewMap = pop(NewMap0),
  {SwitchInsn++[hipe_amd64:mk_fp_unop('fstp', Temp)], NewMap}.

find_a_temp([H = #amd64_temp{}|_])->
  H;
find_a_temp([_|T]) ->
  find_a_temp(T);
find_a_temp([]) ->
...
switch_first(X, Map = [H|_]) ->
  ...
  case Pos of
    ...
    _ ->	    
      ...
      Ins = hipe_amd64:mk_fp_unop(fxch, mk_st(Pos)),
      {[Ins], NewMap}
  end;
switch_first(X, Map) ->
  push(X, Map).
...
is_fp(#amd64_fpreg{}) ->
  true;
is_fp(#amd64_mem{type=Type}) ->
  Type =:= 'double';
is_fp(#amd64_temp{type=Type}) ->
  Type =:= 'double'.
...
%%pp_insn(Op, Src, Dst)->
%%  pp([hipe_amd64:mk_fp_binop(Op, Src, Dst)]).

%%pp([I|Ins])->
%%  hipe_amd64_pp:pp_insn(I),
%%  pp(Ins);
%%pp([]) ->
%%  [].

pseudo_pop(Map) when (length(Map)>0) ->
  Dst = hipe_amd64:mk_new_temp('double'),
...
%% don't have overflow for the extended double precision in the x87.
  pseudo_pop(Dst, St-1, 
	     [hipe_amd64:mk_fp_unop('fxch', mk_st(St-1)),
	      hipe_amd64:mk_fp_unop('fst', Dst),
	      hipe_amd64:mk_fp_unop('fxch', mk_st(St-1))
	      |Acc]);
pseudo_pop(Dst, _St, Acc) ->
  [hipe_amd64:mk_fp_unop('fst', Dst)|Acc].
