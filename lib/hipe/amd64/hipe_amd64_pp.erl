...
%%% $Id$
%%% amd64 pretty-printer

-module(hipe_amd64_pp).
...
pp(Dev, #defun{mfa=MFA, code=Code, data=Data}) ->
  ...
  io:format(Dev, "\t.align 4\n", []),
  hipe_data_pp:pp(Dev, Data, amd64, Fname),
  io:format(Dev, "\n", []).
...
pp_insn(Dev, I, Pre) ->
  case I of
    ...
    #fmove{src=Src, dst=Dst} ->
      ...
      io:format(Dev, "\n", []);
    _ ->
      exit({?MODULE, pp_insn, {"unknown amd64 instruction", I}})
  end.

to_hex(N) ->
  io_lib:format("~.16x", [N, "0x"]).

pp_sdesc(Dev, Pre, #amd64_sdesc{exnlab=ExnLab,fsize=FSize,arity=Arity,live=Live}) ->
...
pp_hard_reg(Dev, Reg) ->
  io:format(Dev, "~s", [hipe_amd64_registers:reg_name(Reg)]).

type_tag('tagged') -> "t";
type_tag('untagged') -> "u";
type_tag('double') -> "d".

pp_temp(Dev, #amd64_temp{reg=Reg, type=Type}) ->
  case Type of
    double ->
      Tag = type_tag(Type),
...
pp_fpreg(Dev, #amd64_fpreg{reg=Reg, pseudo=Pseudo})->
  case Pseudo of
    true -> io:format(Dev, "pseudo_fp(~w)", [Reg]);
    _ -> io:format(Dev, "st(~w)", [Reg])
  end.

pp_imm(Dev, #amd64_imm{value=Value}, Dollar) ->
...
pp_mem(Dev, #amd64_mem{base=Base, off=Off}) ->
  pp_off(Dev, Off),
  case Base of
    [] ->
      [];
