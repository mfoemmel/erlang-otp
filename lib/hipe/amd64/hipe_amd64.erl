...
mk_temp(Reg, Type) -> #amd64_temp{reg=Reg, type=Type, allocatable=true}.
mk_nonallocatable_temp(Reg, Type) -> #amd64_temp{reg=Reg, type=Type, allocatable=false}.
mk_new_temp(Type) ->
    mk_temp(hipe_gensym:get_next_var(amd64), Type).
mk_new_nonallocatable_temp(Type) ->
   mk_nonallocatable_temp(hipe_gensym:get_next_var(amd64), Type).
is_temp(X) -> case X of #amd64_temp{} -> true; _ -> false end.
temp_reg(#amd64_temp{reg=Reg}) -> Reg.
temp_type(#amd64_temp{type=Type}) -> Type.
temp_is_allocatable(#amd64_temp{allocatable=A}) -> A.

mk_imm(Value) -> #amd64_imm{value=Value}.
mk_imm_from_addr(Addr, Type) ->
    mk_imm({Addr, Type}).
mk_imm_from_atom(Atom) ->
    mk_imm(Atom).
is_imm(X) -> case X of #amd64_imm{} -> true; _ -> false end.
%% imm_value(#amd64_imm{value=Value}) -> Value.

mk_mem(Base, Off, Type) -> #amd64_mem{base=Base, off=Off, type=Type}.
%% is_mem(X) -> case X of #amd64_mem{} -> true; _ -> false end.
%% mem_base(#amd64_mem{base=Base}) -> Base.
%% mem_off(#amd64_mem{off=Off}) -> Off.
mem_type(#amd64_mem{type=Type}) -> Type.

%% mk_fpreg(Reg) -> #amd64_fpreg{reg=Reg, pseudo=true}.
mk_fpreg(Reg, Pseudo) -> #amd64_fpreg{reg=Reg, pseudo=Pseudo}.
%% is_fpreg(F) -> case F of #amd64_fpreg{} -> true;_ -> false end.
%% fpreg_is_pseudo(#amd64_fpreg{pseudo=Pseudo}) -> Pseudo.
%% fpreg_reg(#amd64_fpreg{reg=Reg}) -> Reg.

mk_mfa(M, F, A) -> #amd64_mfa{m=M, f=F, a=A}.
%% is_mfa(X) -> case X of #amd64_mfa{} -> true; _ -> false end.
mfa_mfa(#amd64_mfa{m=M, f=F, a=A}) -> {M, F, A}.

mk_prim(Prim) -> #amd64_prim{prim=Prim}.
is_prim(X) -> case X of #amd64_prim{} -> true; _ -> false end.
prim_prim(#amd64_prim{prim=Prim}) -> Prim.

mk_sdesc(ExnLab, FSize, Arity, Live) ->
    #amd64_sdesc{exnlab=ExnLab, fsize=FSize, arity=Arity, live=Live}.
...
%%   Defs = hipe_amd64_defuse:insn_def(I),
%%   Uses = hipe_amd64_defuse:insn_use(I),
