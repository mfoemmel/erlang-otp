...
translate(RTL) ->	% RTL function -> amd64 defun
    ...
    hipe_gensym:set_var(x86, hipe_amd64_registers:first_virtual()),
...
conv_insn(I, Map, Data) ->
	...
	load ->
	    ...
	    I2 = case {hipe_rtl:load_size(I), hipe_rtl:load_sign(I)} of
		     {byte, signed} ->
			 [hipe_x86:mk_movsx(
                            hipe_x86:mk_mem(Src, Off, 'byte'), Dst)];
		     {byte, unsigned} ->
			 [hipe_x86:mk_movzx(
                            hipe_x86:mk_mem(Src, Off, 'byte'), Dst)];
		     {int16, signed} ->
			 [hipe_x86:mk_movsx(
                            hipe_x86:mk_mem(Src, Off, 'int16'), Dst)];
		     {int16, unsigned} ->
			 [hipe_x86:mk_movzx(
                            hipe_x86:mk_mem(Src, Off, 'int16'), Dst)];
		     {int32, signed} ->
			 [hipe_x86:mk_movsx(
                            hipe_x86:mk_mem(Src, Off, 'int32'), Dst)];
		     {int32, unsigned} ->
			 [hipe_x86:mk_movzx(
                            hipe_x86:mk_mem(Src, Off, 'int32'), Dst)];
		     _ ->
			 ...
			 case hipe_x86:is_imm(Src) of
			     false ->
				 [hipe_x86:mk_move(
                                    hipe_x86:mk_mem(Src, Off, Type), Dst)];
			     true ->
	...
	switch ->	% this one also updates Data :-(
	    ...
	    I2 = [hipe_x86:mk_move64(JTabImm, JTabReg),
	...
	fload ->
	    ...
	    {{Src, Map1},_} = conv_src(hipe_rtl:fload_src(I), Map0),
            ...
            I2 = [hipe_x86:mk_fmove(
                    hipe_x86:mk_mem(Src, Off, 'double'),Dst)],
	    {I2, Map2, Data};
	fstore ->
	    ...
	    {{Src, Map1},_} = conv_src(hipe_rtl:fstore_src(I), Map0),
            ...
            I2 = [hipe_x86:mk_fmove(
                    Src, hipe_x86:mk_mem(Dst, Off, 'double'))],	    
	    {I2, Map2, Data};
	fp ->
...
conv_shift(Dst, Src1, BinOp, Src2) ->
  {NewSrc2,I1} =
	...
	{Src2, []};
      false ->
	NewSrc = hipe_x86:mk_temp(hipe_amd64_registers:rcx(), 'untagged'),
...
do_call_results([]) ->
    [];
do_call_results([Dst]) ->
    EAX = hipe_x86:mk_temp(hipe_amd64_registers:rax(), 'tagged'),
	...
	true ->
            ...
            case is_imm64(ImmVal) of
                true ->
                    ...
                    {{Temp, Map}, 
                     [hipe_x86:mk_move64(hipe_x86:mk_imm(ImmVal), Temp)]};
                false ->
...
mk_eax() ->
    hipe_x86:mk_temp(hipe_amd64_registers:rax(), 'tagged').
