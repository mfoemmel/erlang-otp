...
-module(hipe_amd64_frame).
-export([frame/2]).
-include("hipe_amd64.hrl").

frame(Defun, _Options) ->
  Formals = fix_formals(hipe_amd64:defun_formals(Defun)),
  Temps0 = all_temps(hipe_amd64:defun_code(Defun), Formals),
  MinFrame = defun_minframe(Defun),
  Temps = ensure_minframe(MinFrame, Temps0),
  CFG0 = hipe_amd64_cfg:init(Defun),
  Liveness = hipe_amd64_liveness:analyse(CFG0),
  CFG1 = do_body(CFG0, Liveness, Formals, Temps),
  hipe_amd64_cfg:linearise(CFG1).

fix_formals(Formals) ->
  fix_formals(hipe_amd64_registers:nr_args(), Formals).
...
do_blocks(CFG, Context) ->
  Labels = hipe_amd64_cfg:labels(CFG),
  do_blocks(Labels, CFG, Context).

do_blocks([Label|Labels], CFG, Context) ->
  Liveness = context_liveness(Context),
  LiveOut = hipe_amd64_liveness:liveout(Liveness, Label),
  Block = hipe_amd64_cfg:bb(CFG, Label),
  Code = hipe_bb:code(Block),
  NewCode = do_block(Code, LiveOut, Context),
  NewBlock = hipe_bb:code_update(Block, NewCode),
  NewCFG = hipe_amd64_cfg:bb_add(CFG, Label, NewBlock),
...
do_insn(I, LiveOut, Context, FPoff) ->
  case I of
    ...
    _ ->  % comment, jmp, label, pseudo_jcc, pseudo_tailcall_prepare, move64
      {[I], FPoff}
  end.

%%%
%%% Convert any pseudo-temp operand in a binary (alu, cmp, move)
%%% or unary (push) instruction to an explicit amd64_mem operand.
%%%
...
conv_pseudo(Temp, Off) ->
  hipe_amd64:mk_mem(mk_sp(), hipe_amd64:mk_imm(Off), hipe_amd64:temp_type(Temp)).
...
do_ret(_I, Context, FPoff) ->
  %% XXX: this conses up a new ret insn, ignoring the one rtl->amd64 made
  adjust_sp(FPoff, [hipe_amd64:mk_ret(word_size()*context_arity(Context))]).

adjust_sp(N, Rest) ->
  if N =:= 0 ->
      Rest;
     true ->
      [hipe_amd64:mk_alu('add', hipe_amd64:mk_imm(N), mk_sp()) | Rest]
  end.
...
do_pseudo_call(I, LiveOut, Context, FPoff0) ->
  #amd64_sdesc{exnlab=ExnLab,arity=OrigArity} = hipe_amd64:pseudo_call_sdesc(I),
  Fun0 = hipe_amd64:pseudo_call_fun(I),
  Fun1 = conv_opnd(Fun0, FPoff0, Context),
  LiveTemps = [Temp || Temp <- LiveOut, temp_is_pseudo(Temp)],
  SDesc = mk_sdesc(ExnLab, Context, LiveTemps),
  ContLab = hipe_amd64:pseudo_call_contlab(I),
  Linkage = hipe_amd64:pseudo_call_linkage(I),
  CallCode = [hipe_amd64:mk_pseudo_call(Fun1, SDesc, ContLab, Linkage)],
  ...
  StkArity = max(0, OrigArity - hipe_amd64_registers:nr_args()),
...
mk_sdesc(ExnLab, Context, Temps) ->	% for normal calls
  ...
  hipe_amd64:mk_sdesc(ExnLab, FSize div word_size(), Arity,
                    list_to_tuple(Live)).

only_tagged(Temps)->
  [X || X <- Temps, hipe_amd64:temp_type(X) =:= 'tagged'].
...
mk_minimal_sdesc(Context) ->		% for inc_stack_0 calls
  hipe_amd64:mk_sdesc([], 0, context_arity(Context), {}).
...
do_pseudo_tailcall(I, Context) ->	% always at FPoff=context_framesize(Context)
  Arity = context_arity(Context),
  Args = hipe_amd64:pseudo_tailcall_stkargs(I) ++ [context_ra(Context)],
  Fun0 = hipe_amd64:pseudo_tailcall_fun(I),
  ...
  I2 = hipe_amd64:mk_jmp_fun(Fun1, hipe_amd64:pseudo_tailcall_linkage(I)),
...
do_tailcall_args(Args, Context, Fun0) ->
  ...
  {PushFun0, FPoff3, LoadFun1, Fun1} =
	...
	{[], FPoff2, [], Fun0};
      true ->
	Type = hipe_amd64:temp_type(Fun0),
	Temp1 = mk_temp1(Type),
	Fun0Off = context_offset(Context, Fun0),
	MEM0 = conv_pseudo(Fun0, FPoff2 + Fun0Off),
	if Fun0Off >= DangerOff ->
	    Fun1Off = hipe_amd64:mk_imm(0),
	    MEM1 = hipe_amd64:mk_mem(mk_sp(), Fun1Off, Type),
	    {[hipe_amd64:mk_push(MEM0)],
	     FPoff2 + word_size(),
	     [hipe_amd64:mk_move(MEM1, Temp1)],
	     Temp1};
	   true ->
	    {[], FPoff2, [hipe_amd64:mk_move(MEM0, Temp1)], Temp1}
	end
    end,
  %%
  RegTemp0 = hipe_amd64_registers:temp0(),
  TempReg =
    case hipe_amd64:is_temp(Fun1) of
      true ->
	RegFun1 = hipe_amd64:temp_reg(Fun1),
	if RegFun1 =/= RegTemp0 -> RegTemp0;
	   true -> hipe_amd64_registers:temp1()
	end;
      false ->
	RegTemp0
    end,
...
split_conflict([{SrcOff,DstOff,Type}|Conflict], FPoff, Pushes, Simple) ->
  Push = hipe_amd64:mk_push(
           hipe_amd64:mk_mem(mk_sp(), hipe_amd64:mk_imm(FPoff+SrcOff), Type)),
...
simple_moves([{SrcOff,DstOff,Type}|Moves], FPoff, TempReg, Rest) ->
  Temp = hipe_amd64:mk_temp(TempReg, Type),
  SP = mk_sp(),
  LoadOff = hipe_amd64:mk_imm(FPoff+SrcOff),
  LD = hipe_amd64:mk_move(hipe_amd64:mk_mem(SP, LoadOff, Type), Temp),
  StoreOff = hipe_amd64:mk_imm(FPoff+DstOff),
  ST = hipe_amd64:mk_move(Temp, hipe_amd64:mk_mem(SP, StoreOff, Type)),
...
store_moves([{Src,DstOff}|Moves], FPoff, Rest) ->
  ...
  SP = mk_sp(),
  StoreOff = hipe_amd64:mk_imm(FPoff+DstOff),
  ST = hipe_amd64:mk_move(Src, hipe_amd64:mk_mem(SP, StoreOff, Type)),
...
mk_context(Liveness, Formals, Temps) ->
  RA = hipe_amd64:mk_new_temp('untagged'),
...
enter_vars([V|Vs], PrevOff, Map) ->
  Off = PrevOff - word_size(), % XXX: minor diff from x86 here
...
do_prologue(CFG, Context) ->
  ...
  if MaxStack > 0 ->
      ...
      OldStartLab = hipe_amd64_cfg:start_label(CFG),
      NewStartLab = hipe_gensym:get_next_label(amd64),
      AllocFrameLab = hipe_gensym:get_next_label(amd64),
      IncStackLab = hipe_gensym:get_next_label(amd64),
      %%
      Type = 'untagged',
      Preg = hipe_amd64_registers:proc_pointer(),
      Pbase = hipe_amd64:mk_temp(Preg, Type),
      SP_LIMIT_OFF = hipe_amd64:mk_imm(
                        hipe_amd64_registers:sp_limit_offset()),
      Temp0 = mk_temp0(Type),
      SP = mk_sp(),
      NewStartCode =
	%% hopefully this lea is faster than the mov;sub it replaced
	[hipe_amd64:mk_lea(
           hipe_amd64:mk_mem(SP, hipe_amd64:mk_imm(-MaxStack), 'untagged'),
           Temp0),
	 hipe_amd64:mk_cmp(
           hipe_amd64:mk_mem(Pbase, SP_LIMIT_OFF, Type), Temp0),
	 hipe_amd64:mk_pseudo_jcc('b', IncStackLab, AllocFrameLab, 0.01)],
      %%
      AllocFrameCode =
	...
	case FrameSize of
	  0 ->
	    ...
	    [hipe_amd64:mk_jmp_label(OldStartLab)];
	  _ ->
	    [hipe_amd64:mk_alu('sub', hipe_amd64:mk_imm(FrameSize), SP),
	     hipe_amd64:mk_jmp_label(OldStartLab)]
	end,
      %%
      IncStackCode =
	[hipe_amd64:mk_call(hipe_amd64:mk_prim('inc_stack_0'),
			    mk_minimal_sdesc(Context), not_remote),
	 hipe_amd64:mk_jmp_label(NewStartLab)],
      %%
      CFG1 = hipe_amd64_cfg:bb_add(CFG, NewStartLab,
                                 hipe_bb:mk_bb(NewStartCode)),
      CFG2 = hipe_amd64_cfg:bb_add(CFG1, AllocFrameLab,
                                 hipe_bb:mk_bb(AllocFrameCode)),
      CFG3 = hipe_amd64_cfg:bb_add(CFG2, IncStackLab,
                                 hipe_bb:mk_bb(IncStackCode)),
      CFG4 = hipe_amd64_cfg:start_label_update(CFG3, NewStartLab),
      %%
      CFG4;
     true ->
      CFG
...
typeof_src(Src) ->
  case Src of
    #amd64_imm{} ->
      'untagged';
    #amd64_temp{} ->
      hipe_amd64:temp_type(Src);
    #amd64_mem{} ->
      hipe_amd64:mem_type(Src)
  end.

%%% Cons up an '%sp' Temp.

mk_sp() ->
  hipe_amd64:mk_temp(hipe_amd64_registers:sp(), 'untagged').

%%% Cons up a '%temp0' Temp.

mk_temp0(Type) ->
  hipe_amd64:mk_temp(hipe_amd64_registers:temp0(), Type).
...
opnd_is_pseudo(Opnd) ->
  case hipe_amd64:is_temp(Opnd) of
    true -> temp_is_pseudo(Opnd);
    false -> false
  end.

temp_is_pseudo(Temp) ->
  case hipe_amd64:is_temp(Temp) of
    true -> 
...
find_temps([I|Insns], S0) ->
  S1 = tset_add_list(S0, hipe_amd64_defuse:insn_def(I)),
  S2 = tset_add_list(S1, hipe_amd64_defuse:insn_use(I)),
  find_temps(Insns, S2);
find_temps([], S) ->
  S.
...
defun_minframe(Defun) ->
  MaxTailArity = body_mta(hipe_amd64:defun_code(Defun), 0),
  MyArity = length(fix_formals(hipe_amd64:defun_formals(Defun))),
...
insn_mta(I, MTA) ->
  case I of
    #pseudo_tailcall{arity=Arity} ->
      max(MTA, Arity - hipe_amd64_registers:nr_args());
    _ -> MTA
  end.
