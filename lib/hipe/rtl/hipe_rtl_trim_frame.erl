...
add_save_restore(CFG) ->
   ...
   Saves0 = hipe_hash:init(InitSaves),
   Saves = saves_loop(CFG, Saves0, Labels, Liveness),
   StacksOut0 = hipe_hash:init([{L, new_stack()} || L <- Labels]),
   StacksOut1 = stacks_loop(CFG, PredMap, StacksOut0, Saves, Labels, Liveness),

   StacksIn0 = hipe_hash:init([{L, new_stack()} || L <- Labels]),
   StacksIn1 = init_stacksin(Labels, CFG, PredMap, StacksOut1, StacksIn0),

   %% io:format("StacksOut1 ~p~n~n", [hipe_hash:list(StacksOut1)]),
   %% io:format("StacksIn1 ~p~n~n", [hipe_hash:list(StacksIn1)]),

   {StacksIn, StacksOut} = 
      bw_stacks_loop(CFG, PredMap, StacksIn1, StacksOut1, Saves, Liveness,
		     Labels),
   
   %% io:format("StacksIn ~p~n~n", [hipe_hash:list(StacksIn)]),
   %% io:format("StacksOut ~p~n~n", [hipe_hash:list(StacksOut)]),
...
savein(Saves, Lbl) ->
   {found, SaveIn} = hipe_hash:lookup(Lbl, Saves),
   SaveIn.

set_savein(Saves, Lbl, SaveIn) ->
   hipe_hash:update(Lbl, SaveIn, Saves).
...
%   io:format("L ~w\nIn ~w\nOld ~w\nNew ~w\n Eq ~w\n\n",[L,StackIn,OldStackOut, StackOut, StackOut =/= OldStackOut]),
...
stack_instrs([I|Is], StackIn, SaveOut, LiveOut) ->
   case hipe_rtl:type(I) of
	 ...
	 NewSaves = ToSave -- StackIn,%order_saves(ToSave -- StackIn, SaveOut0),
...
stack(Stacks, Lbl) ->
   {found, Stack} = hipe_hash:lookup(Lbl, Stacks),
   Stack.

set_stack(Stacks, Lbl, Stack) ->
   hipe_hash:update(Lbl, Stack, Stacks).
  ...
  {Changed0,
   ...
   StackOut0} = 
	...
	{Changed,
	 ...
	 StackOut};
      false ->
	 StackOut00 = intersect_stacks(StackOut, CalcStackOut),
	 {true, set_stack(StacksOut, L, StackOut00), StackOut00}
    end,
  Code = hipe_bb:code(hipe_rtl_cfg:bb(CFG, L)),
  ...
  {Changed1,
   ...
   StacksIn0} =
    case (NewStackIn =:= OldStackIn) and (NewStackIn =:= CalcStackIn) of
      true ->  {Changed0, StacksOut0, StacksIn};
      false ->
	 StackIn00 = intersect_stacks(NewStackIn, CalcStackIn),
	 ...
	 StacksIn00 = set_stack(StacksIn, L, StackIn00),
	 Succ = hipe_rtl_cfg:succ(hipe_rtl_cfg:succ_map(CFG), L),
	 SaveOut = union_saveins(Succ, Saves),
	 LiveOut = hipe_rtl_liveness:liveout(Liveness, L),
	 StackOut42 = stack_instrs(Code, StackIn00, SaveOut, LiveOut),
	 StacksOut10 = set_stack(StacksOut0, L, StackOut42),
	{true,
	 StacksOut10,
	 StacksIn00}
   end,
...
block_add_save_restore([I], StackIn, StackOut, LiveOut) ->
  case hipe_rtl:type(I) of
    call ->
      ...
      check_saves(Push),
      ToSave = Push,
      TotalNeed = length(Live),
      LocalNeed = length(Push),
	...
	[] -> %% Not in a catch
	  case Pop of
	    [] -> %% Nothing to restore
	      {[hipe_rtl:mk_stackneed(LocalNeed),
		hipe_rtl:mk_save_frame(ToSave),
		I], 
	       TotalNeed, 0, []};
	    _ -> %% A restore is needed 
	...
	_ -> %% In a catch
	  ...
	  NewI = hipe_rtl:call_continuation_update(
		   ...
		   NewCont),
	  SP = hipe_rtl:mk_reg(hipe_sparc_registers:stack_pointer()),
	  IncStack = hipe_rtl:mk_alu(SP, SP, add, hipe_rtl:mk_imm((LocalNeed+1)*4)),
	  FrameCode = save_vars(ToSave, SP, 0, 
				hipe_tagscheme:write_catch_frame(SP, LocalNeed*4, NFail)++
				[IncStack,NewI]),
	  {FrameCode,
	   TotalNeed+1, 0, {Pop,{Cont, NewCont},
		       {Fail,NFail,hipe_rtl:call_dst(NewI), LocalNeed}}}
      end;
    enter ->
      case StackIn of
	[] ->
...
do_not_save() ->
  case get(hipe_do_not_save) of
    undefined ->
      S = 
	ordsets:from_list(
	  lists:map(fun (R) -> hipe_rtl:mk_reg(R) end, 
		    hipe_sparc_registers:global())),
      put(hipe_do_not_save,S),
      S;
    Set -> Set
  end.
