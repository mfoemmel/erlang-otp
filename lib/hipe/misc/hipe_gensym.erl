...
-export([init/0,new_var/0,new_label/0,
	 update_vrange/1,update_lblrange/1,var_range/0,label_range/0,
...
init(What) ->
    ...
    put({What,var_counter}, 0),
    put({What,label_counter}, 0),
    put({What,var_min}, 0),
    put({What,var_max}, 0),
    put({What,lbl_min}, 1),
    put({What,lbl_max}, 1).
...
set_var(X) ->
   put(var_counter, X).
set_var(What,X) ->
   put({What,var_counter}, X).

get_var() ->
   get(var_counter).
get_var(What) ->
   get({What,var_counter}).
...
get_next_var() ->
   ...
   C+1.
get_next_var(What) ->
   C = get({What,var_counter}),
   put({What,var_counter}, C+1),
   C+1.
...
set_label(X) ->
   put(label_counter, X).
set_label(What,X) ->
   put({What,label_counter}, X).

get_label() ->
   get(label_counter).
get_label(What) ->
   get({What,label_counter}).

get_next_label() ->
   C = get(label_counter),
   put(label_counter, C+1),
   C+1.
get_next_label(What) ->
   C = get({What,label_counter}),
   put({What,label_counter}, C+1),
   C+1.
