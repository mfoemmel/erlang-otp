...
%% Notes     : 
%%>----------------------------------------------------------------------<
searchBlockCode([#return{vars=Vars}|_]) ->
  {hasReturn, Vars};
  ...
  case findDefine(lists:reverse(Code), Var) of
	...
	1 ->
	  case lists:member(Label, LabLst) of
	    false ->
	      [Pred] = Preds,
	      traverseCode(Pred, SrcVar, PredMap, IcodeCfg, [Label|LabLst]);
	    true ->
	      {false, none, -1}
	  end;
	_ ->
  ...
  case isCallPrimop(I, mktuple) and (length(DstLst) =:= 0) of
	...
	true ->
	  case (hipe_icode:call_args(I) > 1) of 
	    true ->
	      optimizeDefine(Code, Dst, hipe_icode:call_args(I), Res);
	    false ->
	      {none, noOpt}
	  end;
	false ->
	...
	false ->
	  case lists:member(OldVar, hipe_icode:call_args(I)) of
	    true ->
	      ...
	      %% case Def of
	      %%	true ->
	      removeUnElems(Code, OldVar, DstLst, [I|Res], Def, Lab);
	      %%	false ->
	      %%	    removeUnElems(Code, OldVar, DstLst, 
	      %%			  [I|Res], Def, Lab)
	      %% end;
	...
	false ->
	  case hipe_icode:is_type(I) and not Def of
	    true ->
	      case Lab =:= none of
		true ->
		  NewFalseLab = hipe_gensym:get_next_label(icode);
		false ->
		  NewFalseLab = Lab
	      end,
		...
		false ->
		  case lists:member(OldVar, hipe_icode:defines(I)) of
		    true ->
		      removeUnElems(Code, OldVar, DstLst, [I|Res], true, Lab);
		    false ->
		      removeUnElems(Code, OldVar, DstLst, [I|Res], Def, Lab)
		  end
	      end
