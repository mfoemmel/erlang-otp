...
%% equivalence classes to be merged.
%%
...
union2(X,Y,U) when is_integer(X), is_integer(Y) ->
    {V,NxtU} = find(X,U),
    if V > Y ->
	    vector:set(V,NxtU,Y);
       V < Y ->
