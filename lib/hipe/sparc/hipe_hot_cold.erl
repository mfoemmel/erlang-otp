...
-export([finalize/3,get_hotness/1]).


get_hotness(_MFA) ->
  {-1,[]}.
%% {0.5,absolute_to_rel(hipe_profile:read_counters(_MFA))}.


finalize(CFG,_Hotness,Options) ->
  OptCFG = case proplists:get_bool(sparc_peephole,Options) of
	     true  -> hipe_sparc_cfg:remove_dead_code(CFG);
	     false -> CFG
	   end,
  LinearCode = opt(hipe_sparc:sparc_code(hipe_sparc_cfg:linearize(OptCFG))),
  AllHotCode = fill_delay(LinearCode, Options),
  {OptCFG, [{hot,lists:flatten(AllHotCode)},[{cold,[]}]]}.
...
fill_delay(Code, Options) ->
  Code0 = 
    case proplists:get_bool(sparc_peephole,Options) of
      true  -> peephole(Code);
      false -> Code
    end,
...
split(I,Lname,Is)->
  case hipe_sparc:type(I) of
	...
	FL ->
	  [[hipe_sparc:b_false_label_update(I,[])], 
	   [hipe_sparc:goto_create(FL,[fallthrough])]|
	   split_at_branch(Is)]
      end;
    br -> 
...
%%	  NLname = hipe_sparc:label_name(NL),
	 [[I] | split_at_branch(Is)];
%%	  [[hipe_sparc:goto_create(NLname,[fallthrough])]|
%%	   split_at_branch([NL|Is])]];
	...
	CCL ->
	  [[hipe_sparc:call_link_continuation_update(I,[])],
	   [hipe_sparc:goto_create(CCL,[fallthrough])]|
	   split_at_branch(Is)]
      end;
    label ->
      [[I] | split_at_branch(Is)];
