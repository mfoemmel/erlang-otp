...
color_0(IG, Spill, PhysRegs, SpillIx, SpillLimit, NumNodes, Target,
  ...
  %%
  Low = 
    low_degree_nodes(Nodes, K, NotAllocatable),
  ...
  MustNotSpill = 
   ...
   {select(Stk, Precolored,
	   IG, K, PhysRegs, NumNodes, Target), 
    NewSpillIx}.
...
handle_non_spill([X|Xs], IG, Spill, K, Ix, Stk, Vis, Low, SpillLimit, Target) ->
  ...
  if Degree > K ->
      ...
      handle_non_spill([X|Xs], NewIG, Spill, K, NewIx, NewStk, NewVis,
		       Low ++ NewLow,
		       SpillLimit, Target);
     true ->
...
%spill_costs(Ns,IG,Vis,Spill) -> 
%   [{spill_cost(N,Info,Vis,Spill), N } || {N,Info} <- Ns].

%spill_cost(N,Info,Vis,Spill) ->
%   if
%      visited(N,Vis) ->
%	 visited;
%      true ->
%	 spill_cost_of(N,Spill) / degree(Info)
%   end.

%select_actual_costs([]) -> [];
%select_actual_costs([{visited,N}|Xs]) ->
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Note: there might be a slight gain in separate the two versions
...
liveout(CFG,L, Target) ->
  ordsets:from_list(
    reg_names(Target:liveout(CFG,L), Target)
   ).
