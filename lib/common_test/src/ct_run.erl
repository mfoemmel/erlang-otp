...
auto_compile(TestSuites) ->
    ...
    {_,HelpMakeErrors} =
	lists:foldl(
	  fun({Dir,_Suite}, {Done,Failed}) ->
		  case lists:member(Dir, Done) of
		      false ->
			  Failed1 =
			      case run_make(helpmods, Dir, all, UserInclude) of
				  {error,{make_failed,BadMods}} ->
...
verify_suites(TestSuites) ->
    io:nl(),
    Verify = fun({Dir,Suite},NotFound) ->
		     TestDir = case ct_util:is_test_dir(Dir) of
				   true  -> Dir;
				   false -> filename:join(Dir, "test")
			       end,
		     if Suite == all ->
			     NotFound;
			true ->
				 ...
				 false -> 
				     ...
				     io:format("Suite ~w not found in directory ~s~n", [Suite,Dir]),
...
run_make(TestDir, Mod, UserInclude) ->
    run_make(all_files, TestDir, Mod, UserInclude).
