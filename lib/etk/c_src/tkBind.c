...
#if defined(ETK)
    code = Tcl_GlobalEval(interp, p); /* eval everything at once !! */
#else
...
#define NUM_SIZE 40
	...
	switch (before[1]) {
		...
		if (tkwin != NULL) {
		    cmd = Itk_GetWidgetCommand(tkwin);
		    if (cmd != NULL) {
		        string = Tcl_GetCommandPath(
		            ((TkWindow*)tkwin)->mainPtr->interp, cmd
		        );
		    } else {
		        string = "??";
		    }
...
#define FIELD_SIZE 48
	...
	*p = '\0';
	...
	patPtr->detail.name = Tk_GetUid(q);
	*p = '>';
