...
extern HINSTANCE Etk_GetInstance(void);
...
InitSockets()
{
    ...
    handle = LoadLibrary("wsock32.dll");
    if (handle != NULL) {
    ...
    class.cbClsExtra = 0;
    class.cbWndExtra = 0;
    class.hInstance = Etk_GetInstance();
    ...
    socketWindow = CreateWindowEx(0, (LPCTSTR)socketClass, "TclSocket",
				  ...
				  Etk_GetInstance(), NULL);
    if (socketWindow == NULL) {
	TclWinConvertError(GetLastError());
	UnregisterClass((LPCTSTR)socketClass, Etk_GetInstance());
	goto error;
    }

    Tcl_CreateExitHandler(CleanupSockets, (ClientData)handle);
    return 1;
...
{
    HINSTANCE handle = (HINSTANCE) clientData;
