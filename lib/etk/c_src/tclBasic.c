...
Tcl_CreateInterp()
{
    ...
    for (cmdInfoPtr = builtInCmds; cmdInfoPtr->proc != NULL; cmdInfoPtr++) {
	int new;
	Tcl_HashEntry *hPtr;
    ...
    /* ETK: (void) TclInterpInit((Tcl_Interp *)iPtr); */
 ...
 *	be used in future calls to Tcl_NameOfCommand or Tcl_DeleteCommand2.
 *
 * Side effects:
 ...
 * Tcl_FindCommand --
 *
 *	Find a command in the command table of an interpreter.
 *
 * Results:
 *	The return value is a token for the command, which can
 *	be used in future calls to Tcl_NameOfCommand or Tcl_DeleteCommand2.
...
Tcl_Command
Tcl_FindCommand(interp, cmdName, flags)
    Tcl_Interp *interp;		/* Token for command interpreter (returned
				 * by a previous call to Tcl_CreateInterp). */
    CONST char *cmdName;	/* Name of command. */
    int flags;			/* flags (TCL_LEAVE_ERR_MSG or 0) */
{
	...
	if (flags&TCL_LEAVE_ERR_MSG) {
	    Tcl_AppendResult(interp,"command \"",cmdName,"\" not found",
		    (char *) NULL);
	}
    }
    return cmd;
					 ...
					 * command. */
{
    ...
    cmdPtr = (Command*) Tcl_FindCommand(interp, cmdName, 0);
    if (cmdPtr == NULL) {
	return 0;
    }
 ...
 *	scope qualifiers). For Tcl7.5 this function is just a synonym
 *	for Tcl_GetCommandName.
 *
 * Results:
 *	The return value is the name of the given command.
...
Tcl_GetCommandPath(interp, command)
    ...
    Tcl_Command command;	/* Token for the command, returned by a
				 ...
				 * The command must not have been deleted. */
{
    return Tcl_GetCommandName(interp, command);
}

/*
 *----------------------------------------------------------------------
 ...
 *
 * Side effects:
 *	cmd will no longer be recognized as a valid command for
 *	interp.
 *
 *----------------------------------------------------------------------
 */
...
/* Just handle calls to the functions needed for erltk
** Do not expand varaibles.
** We could possibly remove parsing of curly braces as well
*/
...
#define ARGV_INCR 10
    ...
    /* copy command (we must patch zero) */
    length = strlen(cmd) + 1;
    cmd_copy = (char*) ckalloc(length);
    memcpy(cmd_copy, cmd, length);
    r = p = cmd_copy;
	...
	case '\t':
	    p++; 
	    continue;
	case '"':  /* string constant (strip quotes) */
	    p++;
	    q = r;
	    while(((c = *p) != '\0') && (c != '"')) {
		if (c == '\\') {
		    *r++ = Tcl_Backslash(p, &count);
		    p += count;
		}
		else
		    *r++ = *p++;
	    ...
	    if (c != '"') {
		sprintf(interp->result, "missing closing quote");
		result = TCL_ERROR;
		goto done;
	    }
	    break;
	case '{':  /* string constant (strip braces) */
	    level = 1;
	    p++;
	    q = r;
	    while((c = *p) != '\0') {
	    ...
	    if (c != '}') {
		sprintf(interp->result, "missing closing brace");
		result = TCL_ERROR;
		goto done;
	    }
	    break;
	...
	/* at this point c is either 0 or blank or " or brace */
	*r++ = '\0';
	argv[argc] = q;
	argc++;
	...
	if (argc == argv_sz) {
	    if (argv == fargv) {
		char** tmp;
		tmp = (char**) malloc(sizeof(char**)*(argv_sz+ARGV_INCR));
		memcpy(tmp, argv, sizeof(char**)*argv_sz);
		argv = tmp;
	    }
	    else {
		argv = (char**) ckrealloc(argv, 
					  sizeof(char**)*(argv_sz+ARGV_INCR));
	    }
	    argv_sz += ARGV_INCR;
	}
    }
    ...
    if ((argc > 0) && 
	((hPtr = Tcl_FindHashEntry(&iPtr->commandTable, argv[0])) != NULL)) {
...
#   define NUM_ARGS 10
    ...
    CONST char *src;			/* Points to current character
					 * in cmd. */
    ...
    CONST char *termPtr;		/* Contains character just after the
					 * last one in the command. */
    CONST char *cmdStart;		/* Points to first non-blank char. in
	...
	while (1) {
	    ...
	    result = TclParseWords((Tcl_Interp *) iPtr, (char *) src, flags,
		    maxArgs, (char **) &termPtr, &newArgs, &argv[argc], &pv);
	    src = termPtr;
	    if (result != TCL_OK) {
		ellipsis = "...";
		goto done;
	...
	if (flags & TCL_RECORD_BOUNDS) {
	    iPtr->evalFirst = (char *) cmdStart;
	    iPtr->evalLast = (char *) src-1;
	}
	...
	for (tracePtr = iPtr->tracePtr; tracePtr != NULL;
	    ...
	    *((char *) src) = 0;
	    (*tracePtr->proc)(tracePtr->clientData, interp, iPtr->numLevels,
		    (char *) cmdStart, cmdPtr->proc, cmdPtr->clientData, argc, argv);
	    *((char *) src) = saved;
	}
