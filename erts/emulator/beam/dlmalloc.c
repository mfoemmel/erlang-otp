...
#warning "dlmalloc hasn't been tested on windows!"
#elif defined (VXWORKS) 
#error "dlmalloc hasn't been ported to vxworks!"
#endif

#if defined(USE_THREADS) || defined(_REENTRANT) || defined(_THREAD_SAFE)
#define THREAD_SAFE_MALLOC
#endif
...
* Synopsis of compile-time options:
  ...
  INTERNAL_SIZE_T           (default: size_t)
     ...
     very small chunks.
  __WIN32__                     (default: undefined)
     Define this on MS win (95, nt) platforms to compile in sbrk emulation.
  LACKS_UNISTD_H            (default: undefined if not __WIN32__)
     Define this if your system does not have a <unistd.h>.
  LACKS_SYS_PARAM_H         (default: undefined if not __WIN32__)
...
#ifdef DEBUG
#define assert(X) \
  do { if (!(X)) erl_assert_error(#X, __FILE__, __LINE__); } while (0)
...
/* 
  __WIN32__ causes an emulation of sbrk to be compiled in
  mmap-based options are not currently supported in __WIN32__.
*/
...
#ifdef HAVE_VOID_PTR_SBRK_CONFLICT
#define CHAR_PTR_SBRK HAVE_VOID_PTR_SBRK_CONFLICT
#else
#define CHAR_PTR_SBRK 0
#endif

#if CHAR_PTR_SBRK
#define SBRK_RET char *
#else
#define SBRK_RET void *
#endif

#if __STD_C
extern SBRK_RET     sbrk(ptrdiff_t);
#else
extern SBRK_RET     sbrk();
#endif
...
#ifdef THREAD_SAFE_MALLOC

#define cALLOc			calloc_not_ts
#define fREe			free_not_ts
#define mALLOc			malloc_not_ts
#define mEMALIGn		memalign_not_ts
#define rEALLOc			realloc_not_ts
#define vALLOc			valloc_not_ts
#define pvALLOc			pvalloc_not_ts
#define mALLINFo		mallinfo_not_ts
#define mALLOPt			mallopt_not_ts
#define cFREe			cfree_not_ts
#define mALLOC_USABLE_SIZe	malloc_usable_size_not_ts
#define mALLOC_TRIm		malloc_trim_not_ts
#define mALLOC_STATs		malloc_stats_not_ts
...
#endif /* #ifdef THREAD_SAFE_MALLOC */
...
/* 
  Emulation of sbrk for __WIN32__
  All code within the ifdef __WIN32__ is untested by me.
...
#ifdef MAP_ANONYMOUS
  p = (mchunkptr)mmap(0, size, PROT_READ|PROT_WRITE|PROT_EXEC,
...
static int dlmalloc_initialized = 0;

void init_dlmalloc(void)
{
  if(dlmalloc_initialized)
    return;

#ifdef THREAD_SAFE_MALLOC
  heap_lock = erts_mutex_sys(0);
  if(!heap_lock)
    erl_exit(1, "Not able to initialize heap lock\n");
    
#endif

  dlmalloc_initialized = 1;
}


#ifdef THREAD_SAFE_MALLOC

/* All exported functions use a wrapper which locks/unlocks the
 * heap before/after calls to the actual functions. This may not be the
 * smartest way to lock the heap, but definitely the easiest.
 */
