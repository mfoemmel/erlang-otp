...
extern unsigned int hipe_sparc_call_to_native(Process*);
...
extern unsigned int hipe_sparc_return_to_native(Process*);

/* Emulated code tailcalls native code. */
extern unsigned int hipe_sparc_tailcall_to_native(Process*);

/* Emulated code throws an exception to its native code caller. */
extern unsigned int hipe_sparc_throw_to_native(Process*);
...
hipe_tailcall_to_native(Process *p, unsigned int arity, Eterm reg[])
{
    ...
    if ((nstkargs = arity - NR_ARG_REGS) < 0)
	nstkargs = 0;
    hipe_check_nstack(p, max(nstkargs, SPARC_LEAF_WORDS));
    hipe_push_sparc_params(p, arity, reg);	/* needs nstkargs words */
    return hipe_sparc_tailcall_to_native(p);
}
...
{
    return hipe_sparc_return_to_native(p);
}
...
{
    return hipe_sparc_throw_to_native(p);
}
