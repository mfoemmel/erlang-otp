...
OS_PROCESS(erts) {
  ...
  if(!restart) {
	...
	printf("Will extend heap %u bytes from address %X\n", ext_heap_size, ext_heap_addr);
	heap_extend_heap((void*)ext_heap_addr, ext_heap_size);
	free_buf((union SIGNAL **) &addr_str);
	free_buf((union SIGNAL **) &s);
      } else {
	fprintf(stderr, "Warning: Variable OSE_EXT_HEAP_ADDR not defined!\n");
	free_buf((union SIGNAL **) &s);
      }
    } else {
      if((s = get_env(erl_block, "OSE_EXT_HEAP_ADDR")) != NULL) {
	fprintf(stderr, "Warning: Variable OSE_EXT_HEAP_SIZE not defined!\n");
	free_buf((union SIGNAL **) &s);
      }
    }
  } else
    printf("erts restarting...\n");
  ...
  if((s = get_env(erl_block, "ERL_HEAP_SIZE")) != NULL) {
    ...
    if((addr_str = get_env(erl_block, "ERL_HEAP_ADDR")) != NULL) {
      ...
      printf("Will reserve %d bytes from address %X for Erlang\n", erl_heap_size, erl_heap_addr);
      erl_set_memory_block((int)erl_heap_size, erl_heap_addr, 1, 1, 0);
      free_buf((union SIGNAL **) &addr_str);
      free_buf((union SIGNAL **) &s);
    } else {
      free_buf((union SIGNAL **) &s);
      fprintf(stderr, "Error: ERL_HEAP_ADDR not defined!\n");
      kill_proc(current_process());
    }
  } else {
    fprintf(stderr, "Warning: ERL_HEAP_SIZE and/or ERL_HEAP_ADDR not defined! ");
    fprintf(stderr, "ELIB_MALLOC must not be used!\n");
  }
...
#define ERL_SYNC_SIG 1

static struct erlSync {
  SIGSELECT sig_no;
};
static union erlSyncSig {
  SIGSELECT sig_no;
};

static int start_erl_ose(void) {
  char *start_mode;
  union erlSyncSig *sig;
  /* send sync signal to shell cmd process (so that shell command function may
     return if erts is started detached */
  sig = (union erlSyncSig *)alloc(sizeof(struct erlSync), ERL_SYNC_SIG);
  send((union SIGNAL **)&sig, erlang_starter_);
...
int start_erl(int argc, char **argv) {
  union SIGNAL *sig;
  static const SIGSELECT erl_sync_sig[2] = {1, ERL_SYNC_SIG};
  PROCESS erts_;
  OSENTRYPOINT erts;

  erl_block = get_bid(erl_tmp_);
  fprintf(stderr, "Erlang block id: %li\n", (unsigned long)erl_block);
  fflush(stderr);
  ...
  erts_ = create_process(OS_PRI_PROC, /* processtype */
			 "erts", /* name        */
			 erts,	/* entrypoint  */
			 524288,/* stacksize   */
			 20,	/* priority    */
  ...
  sig = receive((SIGSELECT*)erl_sync_sig);
  free_buf(&sig);
  ...
  if(!start_detached) {	
    ...
    printf("erts started in interactive mode\n");
    attach(NULL, erts_);
...
static void get_parameters(int argc, char** argv)
{
	...
	ose_error("ROOTDIR, EMU and PROGNAME  must be set", 0);
    }
}
...
static int ext_heap(int argc, char **argv) {
  ...
  printf("Will extend heap with %u bytes from address %X\n", ext_heap_size, ext_heap_addr);

  heap_extend_heap((void*)ext_heap_addr, ext_heap_size);
  return 0;
}

static int erl_mem_show(int argc, char **argv) {
  erl_memory_show();
  return 0;
}
...
void erlHooks(void) {
  ...
  shell_add_cmd("erl_mem_show", "erl_mem_show", "show erts heap info", erl_mem_show);
