...
check_names(Conn) ->
    ...
    ?line ok = check_names_cmp(Conn, Reply),
    ok.
...
check_names_cmp([], []) ->
    ok;
check_names_cmp([{Name,Port,Sock} | Conn], [{Name,Port} | Reply]) ->
...
register_many(I, N, Prefix) when I > N ->
...
unregister_many([]) ->
    ok;
unregister_many([{Name,Port,Sock} | Socks]) ->
    case close(Sock) of
	ok ->
